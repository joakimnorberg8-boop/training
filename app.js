(()=>{
'use strict';
// GAYM v73 Social Home + Group Class Mobile Fix: direct friend messaging, group-class logging, cleaner Progress.
...
function social(){
 ...
 shell(`${header()}<h1 class="page-title">Social</h1><section class="section"><article class="card social-profile-hero own">${avatarMarkup(p,'xl')}<div class="social-profile-copy"><h2>${escapeHtml(p.display_name||data.profile.name)}</h2><p>${p.username?`@${escapeHtml(p.username)}`:'Choose a username'}${p.selected_title?` · ${escapeHtml(p.selected_title)}`:''}</p>${p.bio?`<span>${escapeHtml(p.bio)}</span>`:'<span class="subtle">Your feed, messages and gym people live here.</span>'}</div></article><div class="social-stats"><div><strong>${(data.sessions||[]).length}</strong><span>Workouts</span></div><div><strong>${calcStreak()}</strong><span>Streak</span></div><div><strong id="social-friend-count">${socialCache.friends.length}</strong><span>Friends</span></div></div><div class="profile-actions social-main-actions"><button class="secondary" id="find-friends">FIND FRIENDS</button><button class="secondary" id="social-my-profile">EDIT SOCIAL PROFILE</button></div></section>${tabBar}<section class="section" id="social-tab-content"><article class="card loading-card"><div class="cloud-loader"></div><p>Loading your GAYM circle…</p></article></section>`);
 $('#find-friends').onclick=searchPeople;$('#social-my-profile').onclick=openSocialProfileEditor;$$('[data-social-tab]').forEach(b=>b.onclick=()=>{socialTab=b.dataset.socialTab;social()});
