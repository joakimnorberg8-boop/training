...
const ACTIVE_SESSION_RECOVERY_PREFIX='gaymV116ActiveSession:user:';
function activeSessionRecoveryKey(userId){return userId?`${ACTIVE_SESSION_RECOVERY_PREFIX}${userId}`:null}
function saveActiveSessionRecovery(userId,session){if(!userId)return;try{const k=activeSessionRecoveryKey(userId);if(session)localStorage.setItem(k,JSON.stringify(session));else localStorage.removeItem(k)}catch(e){console.error('active session recovery save',e)}}
function loadActiveSessionRecovery(userId){if(!userId)return null;try{return JSON.parse(localStorage.getItem(activeSessionRecoveryKey(userId))||'null')}catch{return null}}
...
function loadForUser(userId){
 ...
 try{
  ...
  const loaded=Object.assign({},structuredClone(defaults),raw?JSON.parse(raw):{});
  const recovery=loadActiveSessionRecovery(userId),closed=new Set((loaded.activeSessionClosedIds||[]).map(String));
  if(recovery?.id&&!closed.has(String(recovery.id))){
   const main=loaded.activeSession;
   if(!main||String(main.id)!==String(recovery.id)||entityUpdatedAt(recovery)>=entityUpdatedAt(main))loaded.activeSession=recovery;
  }
  return loaded;
...
function normalizeLocalData(){
 ...
 const sessionDeleted=new Set(data.sessionDeletedIds);data.sessions=(data.sessions||[]).filter(x=>x?.id&&!sessionDeleted.has(String(x.id)));
 ...
 try{
  ...
  if(previous&&key){
   try{
    ...
    // CRITICAL: preserve the live activeSession object used by the workout screen.
    // Replacing it here makes event handlers keep writing to a stale object after
    // the first save, so later sets/exercises appear on screen but are never persisted.
    const liveActive=data.activeSession;
    const merged=mergeLatestLocalCriticalState(oldData,data);
    if(liveActive?.id&&merged.activeSession?.id&&String(liveActive.id)===String(merged.activeSession.id)){
     merged.activeSession=liveActive;
    }
    data=merged;
...
function save(){
 ...
 if(data.activeSession)saveActiveSessionRecovery(activeDataUserId,data.activeSession);
...
function persistActiveSession(){if(data.activeSession){data.activeSession.modifiedAt=Date.now();saveActiveSessionRecovery(activeDataUserId,data.activeSession);save()}}
