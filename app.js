(()=>{
'use strict';
// GAYM v103: local-first account data + meal builder + shareable custom workouts.
...
function coachMarkup(){const c=getCoachInsight();return `<button class="card coach-home-card ${c.status}" id="coach-why"><span class="coach-home-icon">C</span><span class="coach-home-copy"><small>GAYM COACH · ${escapeHtml(c.topic)}</small><strong>${escapeHtml(c.headline)}</strong><span>${escapeHtml(c.reason)}</span></span><b>›</b></button>`}
...
function openMealBuilder(){if(!mealBuilderDraft)mealBuilderDraft={name:'',mealType:guessMealType(),items:[]};const d=mealBuilderDraft,t=mealBuilderTotals();openSheet(`<div class="sheet-head"><div><p class="eyebrow">Meal builder</p><h2>Build the whole meal</h2></div><button class="sheet-close" data-close>×</button></div><p class="subtle">Add every ingredient first. Nothing is written to today until you press <strong>LOG MEAL</strong>.</p><div class="inline-fields"><div class="field"><label>Meal name</label><input id="mb-name" value="${escapeHtml(d.name||'')}" placeholder="e.g. Pasta & meat sauce"></div><div class="field"><label>Meal</label><select id="mb-type">${mealTypeOptions(d.mealType)}</select></div></div><div class="meal-builder-actions"><button class="secondary" id="mb-scan">SCAN BARCODE</button><button class="secondary" id="mb-search">SEARCH FOOD</button><button class="secondary" id="mb-manual">LABEL VALUES</button></div><div class="meal-builder-items">${d.items.length?d.items.map((x,i)=>{const m=nutritionScale(x.food.per100,x.amount);return `<article class="meal-builder-item"><div><strong>${escapeHtml(x.food.name)}</strong><small>${escapeHtml(x.food.brand||FOOD_SOURCE_LABELS[x.food.source]||x.food.source||'Food')}</small></div><label><input data-mb-amount="${i}" type="number" min="0.1" step="1" value="${x.amount}"><span>${escapeHtml(x.food.unit||'g')}</span></label><b>${m.kcal} kcal</b><button class="text-btn danger" data-mb-remove="${i}">REMOVE</button></article>`}).join(''):`<div class="empty meal-builder-empty"><strong>No ingredients yet</strong>Scan a package, search Matvaretabellen, or enter label values.</div>`}</div><article class="meal-builder-total"><div><span>TOTAL MEAL</span><strong>${Math.round(t.kcal)} kcal</strong></div><p>${n1(t.protein)} g protein · ${n1(t.carbs)} g carbs · ${n1(t.fat)} g fat · ${n1(t.fiber)} g fiber</p></article><div class="sheet-actions"><button class="primary" id="mb-log" ${d.items.length?'':'disabled'}>LOG MEAL</button><button class="secondary" id="mb-save" ${d.items.length?'':'disabled'}>SAVE FOR LATER</button></div>`);
 ...
 $('#mb-save').onclick=()=>{capture();if(!d.items.length)return;const meal={id:uid(),name:d.name||`${d.mealType} meal`,mealType:d.mealType,items:d.items.map(x=>({...structuredClone(x.food),amount:x.amount}))};data.savedMeals=[...(data.savedMeals||[]),meal];save();toast(`${meal.name} saved for later`);openMealBuilder()};
}
...
function openLoggedMeal(mealId){const items=(data.nutrition||[]).filter(n=>String(n.mealId||'')===String(mealId));if(!items.length)return;const name=items[0].mealName||mealTypeFromEntry(items[0],'Meal');openSheet(`<div class="sheet-head"><div><p class="eyebrow">Logged meal</p><h2>${escapeHtml(name)}</h2></div><button class="sheet-close" data-close>×</button></div><div class="meal-builder-items">${items.map(n=>`<article class="meal-builder-item"><div><strong>${escapeHtml(n.name)}</strong><small>${escapeHtml(FOOD_SOURCE_LABELS[n.source]||n.source||'Food')}</small></div><label><input data-logmeal-amount="${n.id}" type="number" min="0.1" step="1" value="${n.amount||100}"><span>${escapeHtml(n.unit||'g')}</span></label><b>${Math.round(+n.kcal||0)} kcal</b><button class="text-btn danger" data-logmeal-remove="${n.id}">REMOVE</button></article>`).join('')}</div><div class="sheet-actions"><button class="primary" id="logged-meal-done">DONE</button></div>`);$$('[data-logmeal-amount]').forEach(el=>el.onchange=()=>{const n=items.find(x=>String(x.id)===String(el.dataset.logmealAmount)),a=+el.value;if(!n?.per100||!a||a<=0)return;Object.assign(n,{amount:n1(a),...nutritionScale(n.per100,a)});save();openLoggedMeal(mealId)});$$('[data-logmeal-remove]').forEach(b=>b.onclick=()=>{removeNutritionWhere(n=>String(n.id)===String(b.dataset.logmealRemove));save();const left=(data.nutrition||[]).some(n=>String(n.mealId||'')===String(mealId));if(left)openLoggedMeal(mealId);else{closeSheet();nutrition()}});$('#logged-meal-done').onclick=()=>{closeSheet();nutrition()}}
// ---------------------------------------------------------------------------
