(()=>{
'use strict';
// GAYM v63 audited: account-safe persistence, recipe sharing fixes, avatar hardening, and cache cleanup.
...
async function openPrivacySettings(){
 ...
 openSheet(`<div class="sheet-head"><div><p class="eyebrow">Privacy</p><h2>You decide what leaves the locker room</h2></div><button class="sheet-close" data-close>×</button></div><label class="auto-target-row"><input id="privacy-discoverable" type="checkbox" ${p?.discoverable!==false?'checked':''}><span><strong>Let people find my profile</strong><small>Allows other GAYM users to search your name or username. Private body and nutrition data is never included.</small></span></label>${row('privacy-workouts','Workout activity',settings?.workouts_visibility||'friends')}${row('privacy-prs','Personal bests',settings?.prs_visibility||'friends')}${row('privacy-streak','Training streak',settings?.streak_visibility||'friends')}<p class="science-note">Weight, body measurements, calories, nutrition logs and BottomCheck stay private and are not part of social sharing.</p><div class="sheet-actions"><button class="primary" id="save-privacy">SAVE PRIVACY</button></div>`);
 $('#save-privacy').onclick=async()=>{const update={workouts_visibility:$('#privacy-workouts').value,workout_history_visibility:$('#privacy-workouts').value,prs_visibility:$('#privacy-prs').value,streak_visibility:$('#privacy-streak').value,updated_at:new Date().toISOString()};const [a,b]=await Promise.all([sb.from('privacy_settings').update(update).eq('user_id',authUser.id),sb.from('profiles').update({discoverable:$('#privacy-discoverable').checked,updated_at:new Date().toISOString()}).eq('id',authUser.id)]);if(a.error||b.error)return toast(a.error?.message||b.error?.message);await syncRecentActivitiesFromLocal();closeSheet();toast('Privacy updated.');};
}
