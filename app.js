...
async function navigateNotification(n){
 if(n.social){await markSocialNotificationRead(n.id);closeSheet();if(n.kind==='friend_request'){socialTab='friends';go('social');return}if(n.kind==='message'&&n.actor_id){closeSheet();go('chat',{userId:n.actor_id,profile:n.actor});return}if(n.metadata?.post_id){await openActivityPostById(n.metadata.post_id);return}if((n.kind==='gym_drag'||n.metadata?.actor_profile)&&n.actor_id&&n.actor_id!==authUser?.id){openUserProfile(n.actor_id);return}if(n.actor_id&&n.actor_id!==authUser?.id){openUserProfile(n.actor_id);return}go('home');return}
...
function cloudRecipeToApp(r,{saved=false}={}){
 return {id:`cloud-${r.id}`,cloudRecipeId:r.id,ownerId:r.owner_id,localId:r.local_id||null,name:r.name||'Recipe',cuisine:r.cuisine||'Other',meal:'Community',custom:false,community:true,savedCommunity:saved,servings:+r.servings||1,time:+r.cook_time||0,kcal:+r.kcal||0,protein:+r.protein||0,carbs:+r.carbs||0,fat:+r.fat||0,fiber:+r.fiber||0,tags:Array.isArray(r.tags)?r.tags:[],ingredients:Array.isArray(r.ingredients)?r.ingredients:[],steps:Array.isArray(r.steps)?r.steps:[],image:r.image_url||RECIPE_FALLBACK,visibility:r.visibility||'private',creator:r.creator||r.profiles||null,createdAt:r.created_at||null};
}
function visibilityLabel(v){return v==='public'?'All of GAYM':v==='friends'?'Friends':'Only me'}
async function uploadRecipeImageToCloud(recipeId,blob){
 if(!sb||!authUser?.id||!blob)return null;const ext=blob.type==='image/png'?'png':blob.type==='image/webp'?'webp':'jpg';const path=`${authUser.id}/${String(recipeId).replace(/[^a-zA-Z0-9_-]/g,'_')}-${Date.now()}.${ext}`;const {error}=await sb.storage.from('recipe-images').upload(path,blob,{upsert:false,contentType:blob.type||'image/jpeg',cacheControl:'31536000'});if(error)throw new Error(`Could not upload recipe photo: ${error.message}`);return sb.storage.from('recipe-images').getPublicUrl(path).data.publicUrl||null;
}
async function upsertOwnedRecipeCloud(r,{forceImage=false,removeImage=false}={}){
 const userId=authUser?.id;if(!sb||!userId||activeDataUserId!==userId)throw new Error('Your account changed. Try again.');
 let existingCloud=null;const found=await sb.from('social_recipes').select('id,image_url,created_at').eq('owner_id',userId).eq('local_id',String(r.id)).maybeSingle();if(found.error)throw found.error;existingCloud=found.data||null;
 let image_url=removeImage?null:(r.image||existingCloud?.image_url||null);const blob=removeImage?null:await getRecipeImageBlob(r.id);if(blob&&(forceImage||!image_url||image_url===RECIPE_FALLBACK))image_url=await uploadRecipeImageToCloud(r.id,blob);
...
async function syncOwnedRecipesFromCloud(expectedUserId=activeDataUserId){
 if(!sb||!authUser||authUser.id!==expectedUserId||activeDataUserId!==expectedUserId)return;
 // Migrate local custom recipes once by upserting them first.
 for(const r of (data.customRecipes||[]).filter(x=>x?.custom&&!x?.sharedFrom&&!x?.community)){
  try{const cloud=await upsertOwnedRecipeCloud({...r,visibility:r.visibility||'private'});r.cloudRecipeId=cloud.id;r.visibility=cloud.visibility||r.visibility||'private';r.image=cloud.image_url||r.image||null}catch(e){console.error('recipe cloud migrate',e)}
 }
 const {data:rows,error}=await sb.from('social_recipes').select('*').eq('owner_id',expectedUserId).order('created_at',{ascending:true});if(error){console.error('load owned recipes',error);return}
 if(authUser?.id!==expectedUserId||activeDataUserId!==expectedUserId)return;
 const byId=new Map((data.customRecipes||[]).filter(x=>x?.custom&&!x?.sharedFrom&&!x?.community).map(x=>[String(x.id),x]));
 for(const r of rows||[]){const id=String(r.local_id||`cloud-owned-${r.id}`),old=byId.get(id)||{};byId.set(id,{...old,id,cloudRecipeId:r.id,name:r.name,cuisine:r.cuisine||'Other',meal:'Custom',custom:true,servings:+r.servings||1,time:+r.cook_time||0,kcal:+r.kcal||0,protein:+r.protein||0,carbs:+r.carbs||0,fat:+r.fat||0,fiber:+r.fiber||0,tags:Array.isArray(r.tags)?r.tags:[],ingredients:Array.isArray(r.ingredients)?r.ingredients:[],steps:Array.isArray(r.steps)?r.steps:[],image:r.image_url||old.image||null,visibility:r.visibility||'private'});}
 const preserved=(data.customRecipes||[]).filter(x=>x?.sharedFrom||x?.community);
...
async function openCommunityRecipe(cloudId){
 let r=[...communityRecipeState.items,...communityRecipeState.saved].find(x=>x.cloudRecipeId===cloudId);if(!r){await loadCommunityRecipes(true);r=[...communityRecipeState.items,...communityRecipeState.saved].find(x=>x.cloudRecipeId===cloudId)}if(!r)return toast('Recipe is no longer available.');
 const isSaved=communityRecipeState.saved.some(x=>x.cloudRecipeId===cloudId),who=r.creator?.display_name||r.creator?.username||'GAYM user';
 openSheet(`<div class="recipe-hero"><img src="${escapeHtml(r.image||RECIPE_FALLBACK)}" alt="${escapeHtml(r.name)}"></div><div class="sheet-head recipe-sheet-head"><div><p class="eyebrow">COMMUNITY · ${escapeHtml(visibilityLabel(r.visibility))}</p><h2>${escapeHtml(r.name)}</h2><p class="subtle">By ${escapeHtml(who)}</p></div><button class="sheet-close" data-close>×</button></div><div class="recipe-tags big">${recipeTags(r).map(t=>`<span>${escapeHtml(t)}</span>`).join('')}</div><div class="recipe-macros"><div><strong>${r.kcal}</strong><span>kcal</span></div><div><strong>${r.protein}g</strong><span>protein</span></div><div><strong>${r.carbs}g</strong><span>carbs</span></div><div><strong>${r.fat}g</strong><span>fat</span></div><div><strong>${r.fiber||0}g</strong><span>fiber</span></div></div><p class="recipe-time">~${r.time||0} min · ${r.servings||1} serving${(r.servings||1)===1?'':'s'} · macros per serving</p><h3 class="recipe-section-title">Ingredients</h3><ul class="ingredient-list">${(r.ingredients||[]).map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul><h3 class="recipe-section-title">Instructions</h3><ol class="step-list">${(r.steps||[]).map(x=>`<li><span>${escapeHtml(x)}</span></li>`).join('')}</ol><div class="sheet-actions"><button class="primary" id="community-save" ${isSaved?'disabled':''}>${isSaved?'SAVED TO MY RECIPES':'SAVE TO MY RECIPES'}</button></div>`);
 const btn=$('#community-save');if(btn&&!isSaved)btn.onclick=async()=>{btn.disabled=true;btn.textContent='SAVING…';const {error}=await sb.from('recipe_favorites').insert({user_id:authUser.id,recipe_id:r.cloudRecipeId});if(error&&error.code!=='23505'){btn.disabled=false;btn.textContent='SAVE TO MY RECIPES';return toast(error.message||'Could not save recipe.')}communityRecipeState.saved=[r,...communityRecipeState.saved.filter(x=>x.cloudRecipeId!==r.cloudRecipeId)].map(x=>({...x,savedCommunity:true}));btn.textContent='SAVED TO MY RECIPES';toast('Recipe saved to My Recipes.')};
}
function nutrition(){
