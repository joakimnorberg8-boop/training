(()=>{
'use strict';
// GAYM v68 Gayer Social: reactions, weekly gay stats, pump checks, workout photos, titles, achievements, gym-drag alerts, Pride mode, Night Out radar, and deep-linked notifications.
...
function nightOutRadarMarkup(){const rows=todaysNightOutFriends();if(!rows.length)return '';return `<article class="card night-radar"><div class="section-head"><h3>WHO'S OUT TONIGHT?</h3><span class="eyebrow">${rows.length} OUT</span></div>${rows.map(p=>`<button data-activity-detail="${p.id}">${avatarMarkup(p.profiles||{},'sm')}<span><strong>${escapeHtml(p.profiles?.display_name||p.profiles?.username||'Friend')}</strong><small>${escapeHtml(p.metadata?.location_type==='home'?'Home':p.metadata?.place||'Out tonight')}</small></span><b>›</b></button>`).join('')}</article>`}
...
async function syncRecentActivitiesFromLocal(expectedUserId=activeDataUserId,{notifyPrs=false}={}){
 ...
 // All legacy standalone PR cards are stale by design in 68-gayer-social. Workout cards are the single social source of truth.
