(()=>{
'use strict';
// GAYM v69 Social UI Fix: visible Friends Night Out radar, reliable radar navigation, and mobile-safe finish/photo sheet.
...
function todaysNightOutFriends(){return (socialCache.feed||[]).filter(p=>p.kind==='night_out'&&p.metadata?.active!==false&&(p.metadata?.date===isoToday()||String(p.created_at||'').slice(0,10)===isoToday())&&p.user_id!==authUser?.id)}
function nightOutRadarMarkup(){const rows=todaysNightOutFriends();return `<article class="card night-radar"><div class="section-head"><div><p class="eyebrow">FRIDAY RADAR</p><h3>WHO'S OUT TONIGHT?</h3></div><span class="eyebrow">${rows.length?`${rows.length} OUT`:'QUIET... FOR NOW'}</span></div>${rows.length?`<div class="night-radar-list">${rows.map(p=>`<button type="button" data-night-radar-post="${p.id}">${avatarMarkup(p.profiles||{},'sm')}<span><strong>${escapeHtml(p.profiles?.display_name||p.profiles?.username||'Friend')}</strong><small>${escapeHtml(p.metadata?.location_type==='home'?'Home tonight':p.metadata?.place||'Out tonight')}</small></span><b>›</b></button>`).join('')}</div>`:`<div class="night-radar-empty"><strong>No one is sharing a Night Out yet.</strong><small>When a friend chooses to share a bar, club or home night, they appear here.</small></div>`}</article>`}
function bindNightOutRadar(){ $$('[data-night-radar-post]').forEach(b=>b.onclick=e=>{e.preventDefault();e.stopPropagation();const post=activityPostCache.get(b.dataset.nightRadarPost)||socialCache.feed.find(p=>p.id===b.dataset.nightRadarPost);if(post)openActivityDetail(post);else openActivityPostById(b.dataset.nightRadarPost)}); }
...
async function enterAuthenticatedAccount(user){
 ...
 authUser=user;entryUnlocked=false;cloudProfile=cachedCloudProfile(userId);socialCache={friends:[],requests:[],feed:[],sharedRecipes:[],notifications:[]};socialCacheUpdatedAt=0;socialLoadPromise=null;
...
function leaveAuthenticatedAccount(){
 ++authEpoch;authUser=null;activeDataUserId=null;cloudProfile=null;data=structuredClone(defaults);normalizeLocalData();entryUnlocked=false;socialCache={friends:[],requests:[],feed:[],sharedRecipes:[],notifications:[]};socialCacheUpdatedAt=0;socialLoadPromise=null;route='home';routeArgs={};
}
...
function socialWorkoutMetadata(sess,detailLevel='full',sharePrs=true,workoutVisibility='friends'){
 ...
 const meta={date:sess.date||null,type:sess.type||'strength',durationMin:Math.max(0,Math.round(Number(sess.durationMin||0))),exerciseCount:(sess.items||[]).length,detail_level:detailLevel,pump:sess.pumpCheck||'',photo_url:(sess.photoVisibility==='friends'&&workoutVisibility==='friends')?(sess.socialPhotoUrl||null):null};
...
async function loadSocialState(force=false){
 ...
 socialLoadPromise=(async()=>{
  const [{data:rels,error:relsError},{data:posts,error:postsError},{data:shares,error:sharesError},{data:notifs,error:notifError}]=await Promise.all([
  ...
  if(relsError)console.error('friendships',relsError);if(postsError)console.error('activity',postsError);if(sharesError)console.error('shares',sharesError);if(notifError&&notifError.code!=='42P01')console.error('notifications',notifError);
  ...
  socialCache.feed=posts||[];socialCache.sharedRecipes=shares||[];socialCache.notifications=notifs||[];
...
async function runPeopleSearch(q){
 q=String(q||'').trim();const box=$('#people-results');if(!box)return;if(q.length<2){box.innerHTML='<div class="empty"><strong>Keep typing</strong>Use at least 2 characters.</div>';return}
 const {data:people,error}=await sb.from('profiles').select('id,username,display_name,avatar_url,bio,selected_title').or(`username.ilike.%${q.replace(/[%(),]/g,'')}%,display_name.ilike.%${q.replace(/[%(),]/g,'')}%`).neq('id',authUser.id).limit(20);
...
function profile(){
 ...
 renderSocialTab();
 loadSocialState(false).then(renderSocialTab).catch(()=>{});
}
