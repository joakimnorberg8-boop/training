(()=>{
'use strict';
// GAYM v72 Social Chat + Group Classes + PB Progress: direct friend messaging, group-class logging, cleaner Progress.
...
async function openDragMeToGym(){
 await loadSocialState(true);if(!socialCache.friends.length)return toast('Add a friend first. Peer pressure requires peers.');
 openSheet(`<div class="sheet-head"><div><p class="eyebrow">DRAG ME TO THE GYM</p><h2>Request homosexual peer pressure</h2></div><button class="sheet-close" data-close>×</button></div><p class="subtle">Pick who gets the emergency. They can tap the notification and land on your profile.</p><div class="social-list">${socialCache.friends.map(f=>`<article class="social-person">${avatarMarkup(f,'md')}<div class="social-person-main"><strong>${escapeHtml(f.display_name||f.username||'Friend')}</strong><span>@${escapeHtml(f.username||'friend')}</span></div><button class="small-btn" data-drag-friend="${f.id}">DRAG ME</button></article>`).join('')}</div>`);
 $$('[data-drag-friend]').forEach(b=>b.onclick=async()=>{b.disabled=true;const who=cloudProfile?.display_name||cloudProfile?.username||'Your friend';const {error}=await sendSocialNotification(b.dataset.dragFriend,'gym_drag','DRAG ME TO THE GYM',`${who} requires homosexual peer pressure.`,{actor_profile:true},`gymdrag:${authUser.id}:${b.dataset.dragFriend}:${isoToday()}`);if(error){b.disabled=false;return toast(error.message||'Could not send it.')}b.textContent='SENT';toast('Peer pressure requested. Godspeed.')});
}
...
async function navigateNotification(n){
 if(n.social){await markSocialNotificationRead(n.id);closeSheet();if(n.kind==='friend_request'){socialTab='friends';go('profile');return}if(n.kind==='message'&&n.actor_id){await openDirectChat(n.actor_id,n.actor);return}if(n.metadata?.post_id){await openActivityPostById(n.metadata.post_id);return}if((n.kind==='gym_drag'||n.metadata?.actor_profile)&&n.actor_id&&n.actor_id!==authUser?.id){openUserProfile(n.actor_id);return}if(n.actor_id&&n.actor_id!==authUser?.id){openUserProfile(n.actor_id);return}go('home');return}
...
function unicornState(){
 ...
 if(night&&night.date===isoToday())return unicornWithSass(UNICORN_STATES.evening,night.partyLine||"Night Out active. The macros have entered witness protection.");
 if(st.active)return unicornWithSass(UNICORN_STATES.pump,voicePick(st.active.beginner?GAYM_VOICE.beginner:GAYM_VOICE.active,`active-${st.active.id}-${isoToday()}`));
 ...
 if(st.sessions.length>1){
  ...
  return unicornWithSass(UNICORN_STATES.pump,`${mix} today. ${voicePick(GAYM_VOICE.multi,`multi-${isoToday()}-${st.sessions.length}`)}`);
 }
 if(st.session){
  ...
  return unicornWithSass(UNICORN_STATES.pump,voicePick(GAYM_VOICE.trained,`trained-${isoToday()}-${st.session.id}`));
 }
...
function renderHomeUnicorn(){
 ...
 const night=nightOutState(),party=night&&night.date===isoToday();return `<div class="hero-art ${u.key} ${party?'party-active':''}" aria-hidden="true"><div class="hero-mascot-row"><div class="hero-mascot-ring"><img class="hero-mascot" src="${u.image}" alt=""><span class="mascot-sparkle mascot-sparkle-a">✦</span><span class="mascot-sparkle mascot-sparkle-b">✧</span></div><div class="unicorn-speech">${escapeHtml(u.sass)}</div></div><span class="mascot-state-label">${escapeHtml(u.label)} · ${unicornPersonalityLevel()}</span></div>`;
}
...
function home(){const today=homeWorkoutState();const week=thisWeekSessions();const min=week.reduce((a,s)=>a+(s.durationMin||0),0);const foods=data.nutrition.filter(n=>n.date===isoToday());const kcal=foods.reduce((a,n)=>a+(+n.kcal||0),0),prot=foods.reduce((a,n)=>a+(+n.protein||0),0);const weightTrend=weightTrendSummary(),latest=weightTrend.current,diff=weightTrend.change.toFixed(1);const days=weekDays();
 ...
 <section class="section gay-social-home"><button class="card drag-gym-card" id="drag-me-gym"><span>DRAG ME TO THE GYM</span><small>Request peer pressure from a friend</small></button></section>
...
function recipeImageSrc(r){if(!r)return RECIPE_FALLBACK;if(!r.custom)return r.image||RECIPE_FALLBACK;return recipeImageUrls.get(recipeImageKey(r.id))||RECIPE_FALLBACK}
...
async function getRecipeImageBlob(id){
 if(!id||!activeDataUserId)return null;
 try{
  const scopedKey=recipeImageKey(id);let blob=await readRecipeImageBlobByKey(scopedKey);
  // One-time compatibility read for photos saved before account-scoped image storage.
  if(!blob){const legacy=await readRecipeImageBlobByKey(String(id));if(legacy instanceof Blob){await putRecipeImage(id,legacy);blob=legacy}}
  return blob instanceof Blob?blob:null;
 }catch(e){console.error('get recipe image blob',e);return null}
}
async function putRecipeImage(id,blob){
 if(!activeDataUserId)throw new Error('No active account for recipe image.');
 if(!(blob instanceof Blob))throw new Error('Recipe image is not a valid image blob.');
 const key=recipeImageKey(id),db=await openRecipeDb();
 try{await new Promise((resolve,reject)=>{const tx=db.transaction(RECIPE_STORE,'readwrite');tx.objectStore(RECIPE_STORE).put(blob,key);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)})}finally{db.close()}
 const cacheKey=key;if(recipeImageUrls.has(cacheKey))URL.revokeObjectURL(recipeImageUrls.get(cacheKey));recipeImageUrls.delete(cacheKey);
}
async function getRecipeImage(id){
 const key=recipeImageKey(id);if(recipeImageUrls.has(key))return recipeImageUrls.get(key);
 const blob=await getRecipeImageBlob(id);if(!blob)return null;const url=URL.createObjectURL(blob);recipeImageUrls.set(key,url);return url;
}
async function deleteRecipeImage(id){
 if(!activeDataUserId)return;const key=recipeImageKey(id);
 try{const db=await openRecipeDb();try{await new Promise((resolve,reject)=>{const tx=db.transaction(RECIPE_STORE,'readwrite');tx.objectStore(RECIPE_STORE).delete(key);tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)})}finally{db.close()}}catch(e){console.error('delete recipe image',e)}
 if(recipeImageUrls.has(key))URL.revokeObjectURL(recipeImageUrls.get(key));recipeImageUrls.delete(key);
}
...
async function enterAuthenticatedAccount(user){
 ...
 authUser=user;entryUnlocked=false;cloudProfile=cachedCloudProfile(userId);socialTab='activity';activityPostCache.clear();socialCache={friends:[],requests:[],feed:[],nightOuts:[],sharedRecipes:[],notifications:[]};socialCacheUpdatedAt=0;socialLoadPromise=null;
...
function leaveAuthenticatedAccount(){
 ++authEpoch;authUser=null;activeDataUserId=null;cloudProfile=null;activityPostCache.clear();socialTab='activity';data=structuredClone(defaults);normalizeLocalData();entryUnlocked=false;socialCache={friends:[],requests:[],feed:[],nightOuts:[],sharedRecipes:[],notifications:[]};socialCacheUpdatedAt=0;socialLoadPromise=null;route='home';routeArgs={};
}
...
async function loadSocialState(force=false){
 ...
 socialLoadPromise=(async()=>{
  const [{data:rels,error:relsError},{data:posts,error:postsError},{data:nightOuts,error:nightOutsError},{data:shares,error:sharesError},{data:notifs,error:notifError}]=await Promise.all([
   ...
   sb.from('recipe_shares').select('id,created_at,shared_by,shared_with,social_recipes(*),profiles:profiles!recipe_shares_shared_by_fkey(username,display_name,avatar_url)').eq('shared_with',userId).order('created_at',{ascending:false}).limit(30),
  ...
  if(relsError)console.error('friendships',relsError);if(postsError)console.error('activity',postsError);if(nightOutsError)console.error('night out radar',nightOutsError);if(sharesError)console.error('shares',sharesError);if(notifError&&notifError.code!=='42P01')console.error('notifications',notifError);
  ...
  socialCache.feed=posts||[];socialCache.nightOuts=nightOuts||[];socialCache.sharedRecipes=shares||[];socialCache.notifications=notifs||[];
...
async function respondFriend(id,status){
 ...
 toast(error?error.message:(status==='accepted'?'Friend added.':'Request declined.'));await loadSocialState(true);profile();
}
async function removeFriend(id){if(!confirm('Remove this friend?'))return;const {error}=await sb.from('friendships').delete().eq('id',id);if(error)return toast(error.message);socialCacheUpdatedAt=0;await loadSocialState(true);profile();toast('Friend removed.')}
...
async function loadDirectMessageThreads(){
 ...
 for(const m of messages){const other=m.sender_id===me?m.recipient_id:m.sender_id;if(seen.has(other))continue;seen.add(other);threads.push({userId:other,last:m,profile:profileMap.get(other)||{id:other,username:'friend'}})}
...
async function openMessagesTab(){
 ...
 box.innerHTML=`<div class="section-head"><h2>Messages</h2><button class="text-btn" id="message-new">+ NEW</button></div><div class="message-thread-list">${state.threads.length?state.threads.map(t=>{const mine=t.last.sender_id===authUser.id,unread=!mine&&!t.last.read_at;return `<button class="card message-thread ${unread?'unread':''}" data-message-user="${t.userId}">${avatarMarkup(t.profile,'md')}<span class="grow"><strong>${escapeHtml(t.profile.display_name||t.profile.username||'Friend')}</strong><small>${mine?'You: ':''}${escapeHtml(String(t.last.body||'').slice(0,75))}</small></span><time>${new Date(t.last.created_at).toLocaleDateString('en-GB',{day:'numeric',month:'short'})}</time></button>`}).join(''):'<div class="empty"><strong>No messages yet</strong>Open a friend and start a conversation.</div>'}</div>`;
 $$('[data-message-user]',box).forEach(b=>b.onclick=()=>openDirectChat(b.dataset.messageUser));
...
async function openNewMessagePicker(){
 ...
 $$('[data-chat-user]').forEach(b=>b.onclick=()=>{closeSheet();openDirectChat(b.dataset.chatUser)});
}
async function openDirectChat(userId,profileHint=null){
 if(!sb||!authUser||!userId)return;
 ...
 const me=authUser.id;
 const {data:msgs,error}=await sb.from('direct_messages').select('id,sender_id,recipient_id,body,read_at,created_at').or(`and(sender_id.eq.${me},recipient_id.eq.${userId}),and(sender_id.eq.${userId},recipient_id.eq.${me})`).order('created_at',{ascending:true}).limit(300);
 if(error)return toast(error.message||'Could not load messages.');
 const unread=(msgs||[]).filter(m=>m.recipient_id===me&&!m.read_at).map(m=>m.id);if(unread.length)await sb.from('direct_messages').update({read_at:new Date().toISOString()}).eq('recipient_id',me).in('id',unread);
 openSheet(`<div class="sheet-head chat-sheet-head"><button class="message-profile-link" id="chat-profile">${avatarMarkup(p,'sm')}<span><small>MESSAGE</small><strong>${escapeHtml(p.display_name||p.username||'Friend')}</strong></span></button><button class="sheet-close" data-close>×</button></div><div class="chat-messages" id="chat-messages">${(msgs||[]).length?(msgs||[]).map(m=>`<div class="chat-bubble ${m.sender_id===me?'mine':'theirs'}"><span>${escapeHtml(m.body)}</span><small>${new Date(m.created_at).toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</small></div>`).join(''):'<div class="empty chat-empty"><strong>Start the chat</strong>Write something with actual words. Revolutionary.</div>'}</div><div class="chat-composer"><textarea id="chat-input" maxlength="1000" rows="2" placeholder="Write a message…"></textarea><button class="primary compact" id="chat-send">SEND</button></div>`);
 const list=$('#chat-messages');if(list)requestAnimationFrame(()=>{list.scrollTop=list.scrollHeight});
 $('#chat-profile').onclick=()=>{closeSheet();openUserProfile(userId)};
 const send=async()=>{const input=$('#chat-input'),body=String(input?.value||'').trim();if(!body)return;const btn=$('#chat-send');btn.disabled=true;const {error}=await sb.from('direct_messages').insert({sender_id:me,recipient_id:userId,body});if(error){btn.disabled=false;return toast(error.message||'Could not send message.')}input.value='';await sendSocialNotification(userId,'message',`${cloudProfile?.display_name||cloudProfile?.username||'A friend'} sent you a message`,body.slice(0,140),{sender_id:me});await loadDirectMessageThreads();openDirectChat(userId,p)};
 $('#chat-send').onclick=send;$('#chat-input').onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}};
...
async function openMessageComposer(userId,p){
 ...
 return openDirectChat(userId,p);
}
...
async function openUserProfile(userId){
 ...
 $('[data-back]').onclick=()=>go('profile');bindActivityCards();$$('[data-profile-workout]').forEach(b=>b.onclick=()=>openActivityDetail(activityPostCache.get(b.dataset.profileWorkout)||posts.find(x=>x.id===b.dataset.profileWorkout)));
...
async function publishRecipeToCloud(r){
 ...
 let image_url=null;
 if(r.custom){
  const blob=await getRecipeImageBlob(r.id);
  if(blob){
   const ext=blob.type==='image/png'?'png':blob.type==='image/webp'?'webp':'jpg';
   const path=`${userId}/${String(r.id).replace(/[^a-zA-Z0-9_-]/g,'_')}-${Date.now()}.${ext}`;
   const {error:uploadError}=await sb.storage.from('recipe-images').upload(path,blob,{upsert:false,contentType:blob.type||'image/jpeg',cacheControl:'31536000'});
   if(uploadError)throw new Error(`Could not upload recipe photo: ${uploadError.message}`);
   image_url=sb.storage.from('recipe-images').getPublicUrl(path).data.publicUrl||null;
  }
 }
...
async function shareRecipeWithFriend(r){
 await loadSocialState(true);if(!socialCache.friends.length)return toast('Add a friend before sharing recipes.');
...
async function saveSharedRecipeLocally(share){
 const userId=authUser?.id;if(!userId||activeDataUserId!==userId)return toast('Your account changed. Try again.');
 const r=share.social_recipes;if(!r)return toast('That shared recipe could not be loaded.');
 const id=`shared-${r.id}`;if(data.customRecipes.some(x=>x.id===id))return toast('Recipe already saved.');
 let photoBlob=null;if(r.image_url){try{const resp=await fetch(r.image_url,{cache:'no-store'});if(!resp.ok)throw new Error(`Photo download failed (${resp.status})`);photoBlob=await resp.blob()}catch(e){console.error('shared recipe photo',e)}}
 const recipe={id,name:r.name,cuisine:r.cuisine||'Other',meal:'Shared',custom:true,servings:r.servings||1,time:r.cook_time||0,kcal:+r.kcal||0,protein:+r.protein||0,carbs:+r.carbs||0,fat:+r.fat||0,fiber:+r.fiber||0,tags:r.tags||[],ingredients:r.ingredients||[],steps:r.steps||[],sharedFrom:share.shared_by,cloudRecipeId:r.id,image:r.image_url||null};
 if(photoBlob){try{await putRecipeImage(id,photoBlob)}catch(e){console.error('save shared recipe image',e);return toast('Could not save the recipe photo. Try again.')}}
 data.customRecipes.push(recipe);
 if(!save()){data.customRecipes=data.customRecipes.filter(x=>x.id!==id);if(photoBlob)await deleteRecipeImage(id);return toast('Could not save recipe on this device.')}
 // Favorite bookkeeping is optional cloud metadata. INSERT avoids requiring UPDATE permission.
 const fav=await sb.from('recipe_favorites').insert({user_id:userId,recipe_id:r.id});if(fav.error&&fav.error.code!=='23505')console.error('recipe favorite sync',fav.error);
 toast('Recipe saved to My Recipes.');
}
function sharedRecipeCard(share){const r=share.social_recipes||{},from=share.profiles||{};return `<article class="card shared-recipe-card"><div><span class="eyebrow">FROM @${escapeHtml(from.username||'friend')}</span><h3>${escapeHtml(r.name||'Recipe')}</h3><p>${Math.round(+r.kcal||0)} kcal · ${Math.round(+r.protein||0)} g protein</p></div><button class="small-btn" data-save-shared="${share.id}">SAVE</button></article>`}

function profile(){
 const p=cloudProfile||{display_name:data.profile.name,username:'',bio:'',avatar_url:null};
 const tabBar=`<div class="social-tabs social-tabs-four"><button data-social-tab="activity" class="${socialTab==='activity'?'active':''}">ACTIVITY</button><button data-social-tab="messages" class="${socialTab==='messages'?'active':''}">MESSAGES<span id="message-unread-tab"></span></button><button data-social-tab="friends" class="${socialTab==='friends'?'active':''}">FRIENDS${socialCache.requests.length?` <b>${socialCache.requests.length}</b>`:''}</button><button data-social-tab="recipes" class="${socialTab==='recipes'?'active':''}">SHARED</button></div>`;
 shell(`${header()}<h1 class="page-title">Profile</h1><section class="section"><article class="card social-profile-hero own">${avatarMarkup(p,'xl')}<div class="social-profile-copy"><h2>${escapeHtml(p.display_name||data.profile.name)}</h2><p>${p.username?`@${escapeHtml(p.username)}`:'Choose a username'}${p.selected_title?` · ${escapeHtml(p.selected_title)}`:''}</p>${p.bio?`<span>${escapeHtml(p.bio)}</span>`:'<span class="subtle">Add a bio so your gym people know what you are training for.</span>'}</div><button class="icon-btn social-edit-btn" id="edit-social-profile">EDIT</button></article><div class="social-stats"><div><strong>${(data.sessions||[]).length}</strong><span>Workouts</span></div><div><strong>${calcStreak()}</strong><span>Streak</span></div><div><strong id="social-friend-count">${socialCache.friends.length}</strong><span>Friends</span></div></div><div class="profile-actions"><button class="secondary" id="find-friends">FIND FRIENDS</button><button class="secondary" id="edit-body-profile">BODY & GOAL</button></div></section>${tabBar}<section class="section" id="social-tab-content"><article class="card loading-card"><div class="cloud-loader"></div><p>Loading your GAYM circle…</p></article></section><section class="section"><p class="eyebrow">Account</p><article class="card" style="padding:0 14px"><button class="settings-row" style="width:100%;background:none;border-left:0;border-right:0;border-top:0;color:inherit;text-align:left" id="privacy-settings"><span>Privacy & sharing</span><small>Friends ›</small></button><button class="settings-row" style="width:100%;background:none;border-left:0;border-right:0;border-top:0;color:inherit;text-align:left" id="notification-settings"><span>Notifications</span><small>GAYM sass ›</small></button><button class="settings-row" style="width:100%;background:none;border-left:0;border-right:0;border-top:0;color:inherit;text-align:left" id="pride-mode-settings"><span>Pride Mode</span><small>${data.prideMode==='on'?'ON':data.prideMode==='off'?'OFF':'AUTO'} ›</small></button>${hasLegacyLocalData()?`<button class="settings-row" style="width:100%;background:none;border-left:0;border-right:0;border-top:0;color:inherit;text-align:left" id="import-legacy-data"><span>Import pre-account data</span><small>ONE TIME ›</small></button>`:''}<button class="settings-row" style="width:100%;background:none;border-left:0;border-right:0;border-top:0;color:inherit;text-align:left" id="export-data"><span>Export local data</span><small>JSON ›</small></button><button class="settings-row danger-text" style="width:100%;background:none;border:0;text-align:left" id="sign-out"><span>Log out</span><small>›</small></button></article></section>`);
 $('#edit-social-profile').onclick=openSocialProfileEditor;$('#find-friends').onclick=searchPeople;$('#edit-body-profile').onclick=openProfileSheet;$('#privacy-settings').onclick=openPrivacySettings;$('#notification-settings').onclick=openNotificationSettings;$('#pride-mode-settings').onclick=openPrideModeSettings;const importLegacy=$('#import-legacy-data');if(importLegacy)importLegacy.onclick=importLegacyLocalData;$('#export-data').onclick=exportData;$('#sign-out').onclick=signOutCloud;$$('[data-social-tab]').forEach(b=>b.onclick=()=>{socialTab=b.dataset.socialTab;profile()});
 renderSocialTab();
 loadSocialState(false).then(renderSocialTab).catch(()=>{});loadDirectMessageThreads().then(st=>{const el=$('#message-unread-tab');if(el&&st.unread)el.innerHTML=` <b>${Math.min(st.unread,9)}${st.unread>9?'+':''}</b>`}).catch(()=>{});
}
function renderSocialTab(){
 ...
 if(socialTab==='activity'){
  ...
  box.innerHTML=`<div class="section-head"><h2>Shared recipes</h2><span class="eyebrow">FROM FRIENDS</span></div><div class="shared-recipe-list">${socialCache.sharedRecipes.length?socialCache.sharedRecipes.map(sharedRecipeCard).join(''):'<div class="empty"><strong>No recipes waiting</strong>When a friend shares one, it lands here.</div>'}</div>`;$$('[data-save-shared]').forEach(b=>b.onclick=()=>{const share=socialCache.sharedRecipes.find(x=>x.id===b.dataset.saveShared);if(share)saveSharedRecipeLocally(share)});
 }
}
