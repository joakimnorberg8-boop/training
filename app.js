...
function renderHomeUnicorn(){
 ...
 const night=nightOutState(),party=night&&night.date===isoToday();return `<div class="hero-art hero-art-full ${u.key} ${party?'party-active':''}" aria-hidden="true"><img class="hero-mascot hero-mascot-full" src="${u.image}" alt=""><div class="hero-image-shade"></div><div class="unicorn-speech unicorn-speech-overlay">${escapeHtml(u.sass)}</div><span class="mascot-state-label">${escapeHtml(u.label)} · ${unicornPersonalityLevel()}</span></div>`;
}
