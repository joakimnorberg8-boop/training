...
function profile(){
 ...
 const settings=$('#privacy-settings')?.parentElement;if(settings){settings.insertAdjacentHTML('afterbegin','<button class="settings-row account-sync-row" style="width:100%;background:none;border-left:0;border-right:0;border-top:0;color:inherit;text-align:left" id="account-sync-now"><span>Account sync<small>Upload this device to your account</small></span><small>SYNC NOW</small></button>');$('#account-sync-now').onclick=forceCloudAccountSync}$('#edit-social-profile').onclick=openSocialProfileEditor;$('#edit-body-profile').onclick=openProfileSheet;$('#privacy-settings').onclick=openPrivacySettings;$('#notification-settings').onclick=openNotificationSettings;$('#pride-mode-settings').onclick=openPrideModeSettings;const importLegacy=$('#import-legacy-data');if(importLegacy)importLegacy.onclick=importLegacyLocalData;$('#export-data').onclick=exportData;$('#sign-out').onclick=signOutCloud;
}
