...
let workoutShareState={loaded:false,loading:false,items:[],error:null,query:''};
function resetWorkoutShareState(){workoutShareState={loaded:false,loading:false,items:[],error:null,query:''}}
...
const defaults={
 ...
 customWorkouts:[],customWorkoutDeletedIds:[],customPrograms:[],sessions:[],sessionDeletedIds:[],activeSessionClosedIds:[],measurements:[],nutrition:[],nutritionDeletedIds:[],foodFavorites:[],savedMeals:[],recipeFavorites:[],customRecipes:[],
...
const ACTIVE_SESSION_RECOVERY_PREFIX='gaymV115ActiveRecovery:user:';
...
function activeSessionRecoveryKey(userId){return userId?`${ACTIVE_SESSION_RECOVERY_PREFIX}${userId}`:null}
function readActiveSessionRecovery(userId){if(!userId)return null;try{return JSON.parse(localStorage.getItem(activeSessionRecoveryKey(userId))||'null')}catch{return null}}
function writeActiveSessionRecovery(userId,session){if(!userId)return false;try{const key=activeSessionRecoveryKey(userId);if(session)localStorage.setItem(key,JSON.stringify(session));else localStorage.removeItem(key);return true}catch(e){console.error('save active session recovery',e);return false}}
function loadForUser(userId){
 if(!userId)return structuredClone(defaults);
 try{
  const raw=localStorage.getItem(accountLocalKey(userId)),loaded=Object.assign({},structuredClone(defaults),raw?JSON.parse(raw):{}),recovery=readActiveSessionRecovery(userId);
  const closed=new Set((loaded.activeSessionClosedIds||[]).map(String));
  if(recovery?.id&&!closed.has(String(recovery.id))){
   const main=loaded.activeSession,mainTime=entityUpdatedAt(main),recoveryTime=entityUpdatedAt(recovery);
   if(!main||String(main.id)===String(recovery.id)&&recoveryTime>=mainTime||String(main.id)!==String(recovery.id)&&recoveryTime>mainTime)loaded.activeSession=structuredClone(recovery);
  }
  return loaded;
...
function buildRecoveryPayload(remotePayload,localPayload=data){
 ...
 const critical=mergeLatestLocalCriticalState(remote,local);
 for(const key of ['nutrition','nutritionDeletedIds','foodFavorites','savedMeals','sessions','sessionDeletedIds','customWorkouts','customWorkoutDeletedIds','activeSession','activeSessionClosedIds'])payload[key]=critical[key];
 return payload;
}
...
function setHasLoggedPerformance(set){return Number(set?.reps)>0}
function repairLoggedSetFlags(session){
 if(!session||session.type==='cardio'||!Array.isArray(session.items))return session;
 let doneSets=0,totalSets=0;
 session.items.forEach(item=>{(item.sets||[]).forEach(set=>{totalSets++;if(set?.done!==true&&set?.manuallySkipped!==true&&setHasLoggedPerformance(set))set.done=true;if(set?.done===true)doneSets++})});
 if(session.completed===true||session.finishedAt){session.doneSets=doneSets;session.totalSets=totalSets}
 return session;
}
function normalizeLocalData(){
 if(data.activeSession){const now=Date.now();if(!Number.isFinite(Number(data.activeSession.startedAt))||Number(data.activeSession.startedAt)<=0)data.activeSession.startedAt=now;data.activeSession.startedAt=Number(data.activeSession.startedAt);data.activeSession.totalPause=Math.max(0,Number(data.activeSession.totalPause)||0);if(data.activeSession.pausedAt!=null)data.activeSession.pausedAt=Number(data.activeSession.pausedAt)||null;repairLoggedSetFlags(data.activeSession);}
 ...
 data.customRecipes=Array.isArray(data.customRecipes)?data.customRecipes:[];data.recipeFavorites=Array.isArray(data.recipeFavorites)?data.recipeFavorites:[];data.foodFavorites=Array.isArray(data.foodFavorites)?data.foodFavorites:[];data.savedMeals=Array.isArray(data.savedMeals)?data.savedMeals:[];data.nutritionDeletedIds=Array.isArray(data.nutritionDeletedIds)?[...new Set(data.nutritionDeletedIds.map(String))]:[];data.sessionDeletedIds=Array.isArray(data.sessionDeletedIds)?[...new Set(data.sessionDeletedIds.map(String))]:[];data.customWorkoutDeletedIds=Array.isArray(data.customWorkoutDeletedIds)?[...new Set(data.customWorkoutDeletedIds.map(String))]:[];data.activeSessionClosedIds=Array.isArray(data.activeSessionClosedIds)?[...new Set(data.activeSessionClosedIds.map(String))].slice(-100):[];data.notifications=Array.isArray(data.notifications)?data.notifications:[];data.bottomCheckins=Array.isArray(data.bottomCheckins)?data.bottomCheckins:[];
 const sessionDeleted=new Set(data.sessionDeletedIds);data.sessions=(data.sessions||[]).filter(x=>x?.id&&!sessionDeleted.has(String(x.id)));data.sessions.forEach(repairLoggedSetFlags);
 const workoutDeleted=new Set(data.customWorkoutDeletedIds);data.customWorkouts=(data.customWorkouts||[]).filter(x=>x?.id&&!workoutDeleted.has(String(x.id)));
 const activeClosed=new Set(data.activeSessionClosedIds);if(data.activeSession?.id&&activeClosed.has(String(data.activeSession.id)))data.activeSession=null;
...
function switchAccountLocalData(userId){activeDataUserId=userId||null;data=loadForUser(activeDataUserId);normalizeLocalData();activeSessionCheckpoint=activeSessionFingerprint(data.activeSession);cloudSyncMeta=loadSyncMeta(activeDataUserId);cloudSyncMeta.lastLocalHash=syncContentHash(data);nutritionSyncLastHash=nutritionSyncHash();persistSyncMeta()}
...
function entityUpdatedAt(item){return Math.max(0,Number(item?.modifiedAt)||0,Number(item?.updatedAt)||0,Number(item?.finishedAt)||0,Number(item?.createdAt)||0,Number(item?.startedAt)||0)}
function mergeEntityArrayById(storedItems,currentItems,deletedIds=[]){
 const deleted=new Set((deletedIds||[]).map(String)),out=[],pos=new Map();
 for(const items of [Array.isArray(storedItems)?storedItems:[],Array.isArray(currentItems)?currentItems:[]]){
  for(const raw of items){if(!raw)return;const id=String(raw.id||'');if(!id||deleted.has(id))continue;const item=structuredClone(raw),i=pos.get(id);if(i==null){pos.set(id,out.length);out.push(item);continue}const prev=out[i],a=entityUpdatedAt(prev),b=entityUpdatedAt(item);if(b>a||(b===a&&items===currentItems))out[i]={...prev,...item};}
 }
 return out;
}
function mergeActiveSessionState(stored,current,closedIds=[]){
 const closed=new Set((closedIds||[]).map(String)),a=stored&&typeof stored==='object'?stored:null,b=current&&typeof current==='object'?current:null;
 if(a?.id&&closed.has(String(a.id)))return b?.id&&!closed.has(String(b.id))?structuredClone(b):null;
 if(b?.id&&closed.has(String(b.id)))return a?.id&&!closed.has(String(a.id))?structuredClone(a):null;
 if(!a)return b?structuredClone(b):null;if(!b)return structuredClone(a);
 if(String(a.id)===String(b.id))return entityUpdatedAt(b)>=entityUpdatedAt(a)?{...structuredClone(a),...structuredClone(b)}:{...structuredClone(b),...structuredClone(a)};
 return entityUpdatedAt(b)>=entityUpdatedAt(a)?structuredClone(b):structuredClone(a);
}
function markSessionDeleted(id){if(!id)return;data.sessionDeletedIds=[...new Set([...(data.sessionDeletedIds||[]).map(String),String(id)])].slice(-1000)}
function markWorkoutDeleted(id){if(!id)return;data.customWorkoutDeletedIds=[...new Set([...(data.customWorkoutDeletedIds||[]).map(String),String(id)])].slice(-1000)}
function markActiveSessionClosed(id){if(!id)return;data.activeSessionClosedIds=[...new Set([...(data.activeSessionClosedIds||[]).map(String),String(id)])].slice(-100)}
function reopenActiveSessionId(id){if(!id)return;data.activeSessionClosedIds=(data.activeSessionClosedIds||[]).filter(x=>String(x)!==String(id));data.sessionDeletedIds=(data.sessionDeletedIds||[]).filter(x=>String(x)!==String(id))}
function activeSessionFingerprint(session){if(!session)return '';const c=structuredClone(session);delete c.modifiedAt;return stableStringify(c)}
let activeSessionCheckpoint='';
function mergeLatestLocalCriticalState(storedPayload,currentPayload=data){
 ...
 const sessionDeleted=[...new Set([...(stored.sessionDeletedIds||[]),...(current.sessionDeletedIds||[])].map(String))].slice(-1000);
 const workoutDeleted=[...new Set([...(stored.customWorkoutDeletedIds||[]),...(current.customWorkoutDeletedIds||[])].map(String))].slice(-1000);
 const activeClosed=[...new Set([...(stored.activeSessionClosedIds||[]),...(current.activeSessionClosedIds||[])].map(String))].slice(-100);
 return {...current,
  ...
  sessionDeletedIds:sessionDeleted,
  sessions:mergeEntityArrayById(stored.sessions,current.sessions,sessionDeleted),
  customWorkoutDeletedIds:workoutDeleted,
  customWorkouts:mergeEntityArrayById(stored.customWorkouts,current.customWorkouts,workoutDeleted),
  activeSessionClosedIds:activeClosed,
  activeSession:mergeActiveSessionState(stored.activeSession,current.activeSession,activeClosed)
 };
}
...
function refreshFromLatestLocal({rerender=false}={}){
 if(!activeDataUserId)return false;try{const raw=localStorage.getItem(accountLocalKey(activeDataUserId));if(!raw)return false;const stored=JSON.parse(raw),before=syncContentHash(data);data=mergeLatestLocalCriticalState(data,stored);normalizeLocalData();activeSessionCheckpoint=activeSessionFingerprint(data.activeSession);const changed=before!==syncContentHash(data);if(changed&&rerender&&entryUnlocked&&document.visibilityState!=='hidden')render();return changed}catch(e){console.error('refresh latest local data',e);return false}
}
...
function syncPlannedWorkoutReferences(){
 ...
 data.customWorkouts.forEach(w=>{if(!['private','friends','public'].includes(w.visibility))w.visibility='private';if(!Array.isArray(w.items))w.items=[]});
...
function save(){
 ...
 const beforeActive=activeSessionFingerprint(data.activeSession);
 if(data.activeSession&&beforeActive!==activeSessionCheckpoint)data.activeSession.modifiedAt=Date.now();
 if(data.activeSession)writeActiveSessionRecovery(activeDataUserId,data.activeSession);else writeActiveSessionRecovery(activeDataUserId,null);
 const nextHash=syncContentHash(data),contentChanged=nextHash!==cloudSyncMeta.lastLocalHash;
 if(contentChanged){data.updatedAt=Date.now();cloudSyncMeta.dirty=true;setCloudSyncState('local')}
 const saved=persistLocalAccountData();
 if(!saved)return false;
 normalizeLocalData();
 activeSessionCheckpoint=activeSessionFingerprint(data.activeSession);
 const persistedHash=syncContentHash(data),persistedNutritionHash=nutritionSyncHash(data),nutritionChanged=persistedNutritionHash!==nutritionSyncLastHash;
 cloudSyncMeta.lastLocalHash=persistedHash;persistSyncMeta();
 if(nutritionChanged){nutritionSyncLastHash=persistedNutritionHash;scheduleNutritionCloudSync()}
 return true;
}
...
function openGroupClassLogger(){
 ...
 $('#save-group-class').onclick=async()=>{const name=$('#group-name').value.trim();if(!name)return toast('Choose or enter a class name.');const date=$('#group-date').value;if(!date||date>isoToday())return toast('Choose a valid date.');const durationMin=Math.max(1,Math.min(300,Number($('#group-duration').value)||45)),startedAt=new Date(`${date}T12:00:00`).getTime(),session={id:uid(),name,type:'group',date,durationMin,groupClass:name,groupCategory:selectedCategory||$('#group-category').value.replace('All','Other'),groupCenter:$('#group-center').value.trim(),groupInstructor:$('#group-instructor').value.trim(),intensity:Number($('#group-intensity').value)||3,notes:$('#group-notes').value.trim(),items:[],doneSets:0,totalSets:0,startedAt,finishedAt:startedAt+durationMin*60000,completed:true,manualEntry:true,updatedAt:Date.now()};data.sessions.push(session);save();await syncRecentActivitiesFromLocal(activeDataUserId);closeSheet();render();toast('Group class logged. Social cardio, but make it scheduled.')};
}

function workoutVisibilityLabel(v){return v==='public'?'All of GAYM':v==='friends'?'Friends':'Only me'}
function workoutCloudPayload(w){
 return {owner_id:authUser.id,local_id:String(w.id),name:String(w.name||'Workout').slice(0,80),type:['strength','rehab','cardio'].includes(w.type)?w.type:'strength',notes:String(w.notes||'').slice(0,1000),items:Array.isArray(w.items)?structuredClone(w.items):[],duration:Math.max(0,Number(w.duration)||0),distance:Math.max(0,Number(w.distance)||0),mode:w.mode||null,visibility:['private','friends','public'].includes(w.visibility)?w.visibility:'private',updated_at:new Date().toISOString()}
}
async function upsertSharedWorkoutCloud(w){
 if(!sb||!authUser?.id||activeDataUserId!==authUser.id)return null;
 const {data:row,error}=await sb.from('social_workouts').upsert(workoutCloudPayload(w),{onConflict:'owner_id,local_id'}).select('id,owner_id,local_id,name,type,notes,items,duration,distance,mode,visibility,created_at,updated_at').single();
 if(error)throw error;return row;
}
async function deleteSharedWorkoutCloud(localId){
 if(!sb||!authUser?.id)return;const {error}=await sb.from('social_workouts').delete().eq('owner_id',authUser.id).eq('local_id',String(localId));if(error)throw error;
}
async function retryPendingWorkoutShares(){if(!sb||!authUser?.id)return;const pending=(data.customWorkouts||[]).filter(w=>w.cloudSyncPending);for(const w of pending){try{await upsertSharedWorkoutCloud(w);w.cloudSyncPending=false;save()}catch(e){console.error('retry workout share',e);break}}}
function cloudWorkoutToApp(row,creator){return {cloudWorkoutId:row.id,ownerId:row.owner_id,name:row.name||'Workout',type:row.type||'strength',notes:row.notes||'',items:Array.isArray(row.items)?row.items:[],duration:Number(row.duration)||0,distance:Number(row.distance)||0,mode:row.mode||'',visibility:row.visibility||'private',creator:creator||null,createdAt:row.created_at||null}}
async function loadCommunityWorkouts(force=false){
 if(workoutShareState.loading||(!force&&workoutShareState.loaded)||!sb||!authUser?.id)return;
 workoutShareState.loading=true;workoutShareState.error=null;const userId=authUser.id;
 try{
  const res=await sb.from('social_workouts').select('id,owner_id,local_id,name,type,notes,items,duration,distance,mode,visibility,created_at,updated_at').neq('owner_id',userId).order('updated_at',{ascending:false}).limit(100);
  if(res.error)throw res.error;if(authUser?.id!==userId)return;
  const rows=res.data||[],ownerIds=[...new Set(rows.map(r=>r.owner_id).filter(Boolean))];let creators=[];
  if(ownerIds.length){const pr=await sb.from('profiles').select('id,username,display_name,avatar_url').in('id',ownerIds);if(pr.error)console.error('workout creators',pr.error);creators=pr.data||[]}
  const byId=Object.fromEntries(creators.map(p=>[p.id,p]));workoutShareState.items=rows.map(r=>cloudWorkoutToApp(r,byId[r.owner_id]));workoutShareState.loaded=true;
 }catch(e){console.error('loadCommunityWorkouts',e);workoutShareState.error=e?.message||'Could not load shared workouts.'}finally{workoutShareState.loading=false}
}
function sharedWorkoutCard(w){const c=w.creator||{},saved=(data.customWorkouts||[]).some(x=>String(x.sourceCloudWorkoutId||'')===String(w.cloudWorkoutId));const meta=w.type==='cardio'?`${w.duration||30} min${w.distance?` · ${w.distance} km`:''}`:`${w.items?.length||0} exercises`;return `<button class="list-card shared-workout-template-card" data-shared-workout="${w.cloudWorkoutId}"><span class="badge-icon ${w.type==='cardio'?'cardio':w.type==='rehab'?'rehab':''}">${w.type==='cardio'?'↗':w.type==='rehab'?'R':'S'}</span><span class="grow"><span class="shared-template-kicker">${w.visibility==='friends'?'FROM A FRIEND':'ALL OF GAYM'}</span><h3>${escapeHtml(w.name)}</h3><p>${escapeHtml(meta)} · by ${escapeHtml(c.display_name||c.username||'GAYM user')}${saved?' · Saved':''}</p></span><span class="chev">›</span></button>`}
function openSharedWorkout(cloudId){const w=workoutShareState.items.find(x=>String(x.cloudWorkoutId)===String(cloudId));if(!w)return toast('Workout is no longer available.');const c=w.creator||{},saved=(data.customWorkouts||[]).some(x=>String(x.sourceCloudWorkoutId||'')===String(cloudId));const rows=w.type==='cardio'?`<article class="workout-detail-privacy"><strong>${escapeHtml(w.mode||w.name)}</strong><span>${w.duration||30} min${w.distance?` · ${w.distance} km`:''}</span></article>`:`<div class="shared-workout-exercises">${(w.items||[]).map((x,i)=>`<article class="shared-exercise"><div><span>${i+1}</span><strong>${escapeHtml(x.name||'Exercise')}</strong></div><small>${Math.max(1,Number(x.sets)||3)} sets${x.reps?` · ${escapeHtml(String(x.reps))} reps`:''}</small></article>`).join('')}</div>`;openSheet(`<div class="sheet-head"><div><p class="eyebrow">${w.visibility==='friends'?'SHARED WITH FRIENDS':'ALL OF GAYM'}</p><h2>${escapeHtml(w.name)}</h2><p class="subtle">by ${escapeHtml(c.display_name||c.username||'GAYM user')}</p></div><button class="sheet-close" data-close>×</button></div>${w.notes?`<p class="subtle shared-workout-notes">${escapeHtml(w.notes)}</p>`:''}${rows}<div class="sheet-actions"><button class="primary" id="save-shared-workout" ${saved?'disabled':''}>${saved?'SAVED TO MY WORKOUTS':'SAVE TO MY WORKOUTS'}</button></div>`);const b=$('#save-shared-workout');if(b&&!saved)b.onclick=()=>saveSharedWorkout(w,b)}
function saveSharedWorkout(w,btn){if((data.customWorkouts||[]).some(x=>String(x.sourceCloudWorkoutId||'')===String(w.cloudWorkoutId)))return toast('Workout already saved.');const copy={id:uid(),name:w.name,type:w.type,notes:w.notes||'',items:structuredClone(w.items||[]),visibility:'private',sourceCloudWorkoutId:w.cloudWorkoutId,sourceOwnerId:w.ownerId,createdAt:Date.now(),updatedAt:Date.now()};if(w.type==='cardio'){copy.duration=w.duration||30;copy.distance=w.distance||0;copy.mode=w.mode||w.name}data.customWorkouts.push(copy);if(!save()){data.customWorkouts=data.customWorkouts.filter(x=>x.id!==copy.id);return toast('Could not save workout.')}btn.disabled=true;btn.textContent='SAVED TO MY WORKOUTS';closeSheet();workout();toast('Workout saved to Your workouts.')}
function paintCommunityWorkouts(){const host=$('#community-workout-list');if(!host)return;const q=String(workoutShareState.query||'').trim().toLowerCase();const items=(workoutShareState.items||[]).filter(w=>!q||`${w.name} ${w.type} ${w.creator?.display_name||''} ${w.creator?.username||''} ${(w.items||[]).map(x=>x.name).join(' ')}`.toLowerCase().includes(q));host.innerHTML=workoutShareState.error?`<div class="empty"><strong>Shared workouts could not load</strong>${escapeHtml(workoutShareState.error)}<button class="secondary compact" id="shared-workout-retry">TRY AGAIN</button></div>`:workoutShareState.loading&&!workoutShareState.loaded?`<article class="card loading-card"><div class="cloud-loader"></div><p>Loading workouts…</p></article>`:items.length?items.map(sharedWorkoutCard).join(''):`<div class="empty"><strong>No shared workouts yet</strong>Workouts shared with Friends or All of GAYM will appear here.</div>`;$$('[data-shared-workout]').forEach(b=>b.onclick=()=>openSharedWorkout(b.dataset.sharedWorkout));const retry=$('#shared-workout-retry');if(retry)retry.onclick=()=>{workoutShareState.loaded=false;loadCommunityWorkouts(true).then(()=>{if(route==='workout')paintCommunityWorkouts()})}}
...
function workout(){
 ...
 <section class="section community-workout-section" id="workouts-from-users"><div class="section-head"><div><p class="eyebrow">FROM GAYM USERS</p><h2>Workouts from other users</h2></div><button class="text-btn" id="refresh-shared-workouts">REFRESH</button></div><p class="subtle">Browse workouts shared by friends or the GAYM community. Saving creates your own private copy.</p><div class="shared-workout-search"><input id="shared-workout-search" value="${escapeHtml(workoutShareState.query||'')}" placeholder="Search workouts, exercises or users..."></div><div class="list shared-workout-template-list" id="community-workout-list"></div></section>`);
 ...
 $$('[data-filter]').forEach(b=>b.onclick=()=>{filter=b.dataset.filter;$$('[data-filter]').forEach(x=>x.classList.toggle('active',x===b));paint()});const sharedSearch=$('#shared-workout-search');if(sharedSearch)sharedSearch.oninput=e=>{workoutShareState.query=e.target.value;paintCommunityWorkouts()};const sharedRefresh=$('#refresh-shared-workouts');if(sharedRefresh)sharedRefresh.onclick=()=>{workoutShareState.loaded=false;loadCommunityWorkouts(true).then(()=>{if(route==='workout')paintCommunityWorkouts()})};paintCommunityWorkouts();retryPendingWorkoutShares().finally(()=>loadCommunityWorkouts(false).then(()=>{if(route==='workout')paintCommunityWorkouts()}));
}
...
function currentBeginnerEquipment(){return Array.isArray(data.beginnerEquipment)?data.beginnerEquipment:['dumbbells','barbell','mat']}
...
function openBeginnerEquipment(){
 ...
 $('#save-beginner-equipment').onclick=()=>{const before=new Set(beginnerAvailableHomeExercises().map(x=>x.key)),picked=$$('.beginner-equipment-list input:checked').map(x=>x.value);data.beginnerEquipment=picked;const after=beginnerAvailableHomeExercises(),unlocked=after.filter(x=>!before.has(x.key));save();closeSheet();beginnerWorkout('workouts');toast(unlocked.length?`${unlocked.length} new beginner exercise${unlocked.length===1?'':'s'} available`:(picked.length?'Home setup updated':'Bodyweight-only setup saved'))};
}
...
function resolvedBeginnerHomeProgram(key){
 const slots=BEGINNER_HOME_SLOTS[key]||[],used=new Set(),resolved=[];
 for(const slot of slots){
  const supported=slot.preferred.filter(ex=>BEGINNER_EXERCISES[ex]&&beginnerEquipmentSupports(ex));
  const picked=supported.find(ex=>!used.has(ex))||supported[0]||null;
  if(picked){used.add(picked);resolved.push(picked)}
 }
 return resolved;
}
function beginnerProgramCoverage(key){
 const slots=BEGINNER_HOME_SLOTS[key]||[],resolved=resolvedBeginnerHomeProgram(key);
 return {resolved,total:slots.length,complete:resolved.length===slots.length,missing:slots.length-resolved.length};
}
...
function startBeginnerProgram(key){
 ...
 const keys=key.startsWith('home')?resolvedBeginnerHomeProgram(key):p.items;if(key.startsWith('home')&&!keys.length)return toast('No compatible exercises for your current home setup');
 startWorkoutTemplate({id:`beginner-${key}`,name:p.name,type:'strength',program:'Beginner',programDay:key,beginner:true,notes:key.startsWith('home')?'Beginner guided workout · adapted to saved home equipment':'Beginner guided workout',items:keys.map(beginnerItem).filter(Boolean)});
}
...
function beginnerWorkoutsMarkup(){return `<section class="section"><div class="section-head"><div><p class="eyebrow">Home workouts</p><h2>Built for your setup</h2></div><button class="text-btn" id="beginner-equipment-button">HOME SETUP</button></div><div class="beginner-setup-summary"><strong>${currentBeginnerEquipment().length?currentBeginnerEquipment().map(x=>x==='dumbbells'?'Dumbbells':x==='barbell'?'Barbell':x==='mat'?'Mat':x==='pullup'?'Pull-up bar':x==='bands'?'Bands':x==='bench'?'Bench':x).join(' · '):'Bodyweight only'}</strong><span>Home A/B only use exercises your setup supports.</span></div><div class="beginner-program-grid">${['homeA','homeB'].map(k=>{const p=BEGINNER_PROGRAMS[k],coverage=beginnerProgramCoverage(k),resolved=coverage.resolved;return `<article class="card beginner-program-card"><div class="beginner-program-title-row"><span class="eyebrow">${p.tag}</span><span class="setup-fit ${coverage.complete?'good':'warn'}">${coverage.complete?'SETUP MATCH':`${coverage.missing} SLOT${coverage.missing===1?'':'S'} MISSING`}</span></div><h3>${p.name}</h3><p>Full body · adapted to your saved home equipment.</p><div class="beginner-ex-list">${resolved.map((x,i)=>`<button type="button" data-beginner-guide="${x}"><span>${i+1}</span><span><strong>${escapeHtml(BEGINNER_EXERCISES[x].name)}</strong><small>${escapeHtml(BEGINNER_EXERCISES[x].equipment)} · ${escapeHtml(BEGINNER_EXERCISES[x].sets+' × '+BEGINNER_EXERCISES[x].reps)}</small></span><span class="chev">›</span></button>`).join('')}${coverage.complete?'':`<div class="beginner-missing-slot"><strong>Your current setup cannot fill every movement slot safely.</strong><span>Add equipment in Home Setup or use the exercises shown above.</span></div>`}</div><button class="primary" data-start-beginner="${k}" ${resolved.length?'':'disabled'}>START ${k==='homeA'?'A':'B'}</button></article>`}).join('')}<article class="card beginner-program-card"><span class="eyebrow">GYM</span><h3>${BEGINNER_PROGRAMS.gym.name}</h3><p>${BEGINNER_PROGRAMS.gym.description}</p><div class="beginner-ex-list">${BEGINNER_PROGRAMS.gym.items.map((x,i)=>`<button type="button" data-beginner-guide="${x}"><span>${i+1}</span><span><strong>${escapeHtml(BEGINNER_EXERCISES[x].name)}</strong><small>${escapeHtml(BEGINNER_EXERCISES[x].sets+' × '+BEGINNER_EXERCISES[x].reps)}</small></span><span class="chev">›</span></button>`).join('')}</div><button class="primary" data-start-beginner="gym">START GYM WORKOUT</button></article></div></section>`}
function beginnerExercisesMarkup(){const all=allGuideExercises();return `<section class="beginner-library-section"><div class="beginner-library-tools"><label><span>⌕</span><input id="beginner-library-search" placeholder="Search exercises..."></label><select id="beginner-setup-filter"><option value="all">All equipment</option><option value="home">My home setup</option></select><select id="beginner-muscle-filter"><option>All muscles</option>${['Chest','Back','Shoulders','Arms','Legs','Core','Glutes'].map(x=>`<option>${x}</option>`).join('')}</select></div><div class="beginner-library-list" id="beginner-library-list">${all.map(beginnerExerciseCard).join('')}</div></section>`}
function beginnerEquipmentMarkup(){const eq=currentBeginnerEquipment();return `<section class="section"><article class="card beginner-equipment-hub"><span class="eyebrow">YOUR HOME SETUP</span><h2>${eq.length?`${eq.length} equipment choices saved`:'Bodyweight only'}</h2><p>${eq.length?eq.map(x=>x==='dumbbells'?'Dumbbells':x==='barbell'?'Barbell + plates':x==='mat'?'Exercise mat':x==='pullup'?'Pull-up bar':x==='bands'?'Resistance bands':x==='bench'?'Bench':x).join(' · '):'No equipment selected. Home workouts will use bodyweight-compatible exercises only.'}</p><button class="primary" id="beginner-edit-equipment">EDIT HOME SETUP</button></article><article class="card beginner-note"><span class="eyebrow">GYM MODE</span><strong>Machine exercises stay available.</strong><p>Your home setup controls Home A/B only. The full exercise library still shows gym exercises so you can learn or use them elsewhere.</p></article></section>`}
function beginnerWorkout(mode='exercises'){
 ...
 const search=$('#beginner-library-search'),filter=$('#beginner-muscle-filter'),setupFilter=$('#beginner-setup-filter'),paint=()=>{const q=(search?.value||'').toLowerCase(),f=filter?.value||'All muscles',homeOnly=setupFilter?.value==='home',all=allGuideExercises().filter(ex=>{const hay=`${ex.name} ${ex.muscle} ${ex.equipment}`.toLowerCase(),group=normalizeMuscle(ex.muscle),key=ex.beginnerGuideKey||guideKeyFromName(ex.name),setupOk=!homeOnly||(key&&beginnerEquipmentSupports(key));return setupOk&&hay.includes(q)&&(f==='All muscles'||group===f||(f==='Arms'&&/bicep|tricep|forearm/i.test(ex.muscle))||(f==='Legs'&&/quad|hamstring|calf|leg|knee/i.test(ex.muscle))||(f==='Glutes'&&/glute|hip/i.test(ex.muscle)))});const list=$('#beginner-library-list');if(list)list.innerHTML=all.length?all.map(beginnerExerciseCard).join(''):'<div class="empty"><strong>No exercises found</strong>Try another search or muscle filter.</div>';$$('[data-guide-name]').forEach(b=>b.onclick=()=>openExerciseGuideByName(b.dataset.guideName,null,guideKeyFromName(b.dataset.guideName)))};
 if(search)search.oninput=paint;if(filter)filter.onchange=paint;if(setupFilter)setupFilter.onchange=paint;paint();
}
...
function openWorkoutSheet(id){const w=data.customWorkouts.find(w=>w.id===id);if(!w)return;openSheet(`<div class="sheet-head"><div><p class="eyebrow">${w.type}</p><h2>${escapeHtml(w.name)}</h2></div><button class="sheet-close" data-close>×</button></div><p class="subtle">${w.type==='cardio'?`${w.duration||30} min${w.distance?` · ${w.distance} km`:''}`:`${w.items?.length||0} exercises`} · ${escapeHtml(workoutVisibilityLabel(w.visibility||'private'))}</p><div class="list">${(w.items||[]).map((x,i)=>`<div class="list-card"><span class="badge-icon">${i+1}</span><span class="grow"><h3>${escapeHtml(x.name)}</h3><p>${x.sets||3} sets${x.reps?` · ${x.reps} reps`:''}</p></span></div>`).join('')}</div><div class="sheet-actions"><button class="primary" id="sheet-start">${w.type==='cardio'?'LOG CARDIO':'START WORKOUT'}</button><button class="secondary" id="sheet-edit">EDIT / SHARE</button><button class="danger-btn" id="sheet-delete">DELETE</button></div>`);$('#sheet-start').onclick=()=>{closeSheet();startWorkout(id)};$('#sheet-edit').onclick=()=>{closeSheet();w.type==='cardio'?openCardioTemplateBuilder(id):openBuilder(w.type,id)};$('#sheet-delete').onclick=async()=>{markWorkoutDeleted(id);data.customWorkouts=data.customWorkouts.filter(x=>x.id!==id);data.planned=data.planned.filter(x=>x.workoutId!==id);save();try{await deleteSharedWorkoutCloud(id)}catch(e){console.error('delete shared workout',e)}resetWorkoutShareState();closeSheet();render();toast('Workout deleted')}}
function openCardioTemplateBuilder(editId){const old=data.customWorkouts.find(w=>w.id===editId);if(!old)return;openSheet(`<div class="sheet-head"><div><p class="eyebrow">Edit cardio template</p><h2>${escapeHtml(old.name)}</h2></div><button class="sheet-close" data-close>×</button></div><div class="field"><label>Workout name</label><input id="cardio-template-name" maxlength="40" value="${escapeHtml(old.name||'')}"></div><div class="field"><label>Cardio mode</label><select id="cardio-template-mode">${CARDIO_ACTIVITIES.map(m=>`<option ${m===(old.mode||old.name)?'selected':''}>${m}</option>`).join('')}</select></div><div class="inline-fields"><div class="field"><label>Duration (min)</label><input id="cardio-template-duration" type="number" min="1" value="${old.duration||30}"></div><div class="field"><label>Distance (km)</label><input id="cardio-template-distance" type="number" min="0" step="0.1" value="${old.distance||''}"></div></div><div class="field"><label>Notes</label><textarea id="cardio-template-notes" rows="4" maxlength="500">${escapeHtml(old.notes||'')}</textarea></div>${workoutVisibilityPicker(old.visibility||'private')}<div class="sheet-actions"><button class="primary" id="cardio-template-save">SAVE CHANGES</button></div>`);$('#cardio-template-save').onclick=async()=>{const name=$('#cardio-template-name').value.trim();if(!name)return toast('Give the workout a name');const updated={...old,name,mode:$('#cardio-template-mode').value,duration:Math.max(1,+$('#cardio-template-duration').value||30),distance:Math.max(0,+$('#cardio-template-distance').value||0),notes:$('#cardio-template-notes').value.trim(),visibility:selectedWorkoutVisibility()};data.customWorkouts=data.customWorkouts.map(w=>w.id===editId?updated:w);syncPlannedWorkoutReferences();save();try{if(updated.visibility!=='private'||old.visibility!=='private')await upsertSharedWorkoutCloud(updated);updated.cloudSyncPending=false}catch(e){console.error('workout share sync',e);updated.cloudSyncPending=true;save();if(updated.visibility!=='private')toast('Saved locally. Sharing could not sync yet.')}resetWorkoutShareState();closeSheet();render();toast('Workout updated everywhere')}}
function workoutVisibilityPicker(selected='private'){selected=['private','friends','public'].includes(selected)?selected:'private';return `<p class="eyebrow form-divider">Who can see this workout?</p><div class="recipe-visibility-picker workout-visibility-picker"><label><input type="radio" name="workout-visibility" value="private" ${selected==='private'?'checked':''}><span><strong>Only me</strong><small>Private custom workout</small></span></label><label><input type="radio" name="workout-visibility" value="friends" ${selected==='friends'?'checked':''}><span><strong>Friends</strong><small>Accepted GAYM friends can find it</small></span></label><label><input type="radio" name="workout-visibility" value="public" ${selected==='public'?'checked':''}><span><strong>All of GAYM</strong><small>Visible under Workouts from other users</small></span></label></div>`}
function selectedWorkoutVisibility(){return $('input[name="workout-visibility"]:checked')?.value||'private'}
function openBuilder(type='strength',editId=null,seedItems=null,seedDraft=null){if(type==='cardio'&&editId)return openCardioTemplateBuilder(editId);if(type==='cardio')return openLogCardio();const old=editId?data.customWorkouts.find(w=>w.id===editId):null;let items=seedItems?seedItems.map(x=>({...x})):(old?.items?.map(x=>({...x}))||[]);const draft=seedDraft||{};openSheet(`<div class="sheet-head"><div><p class="eyebrow">${old?'Edit':'Create'} ${type}</p><h2>${old?escapeHtml(old.name):'New workout'}</h2></div><button class="sheet-close" data-close>×</button></div><div class="field"><label>Workout name</label><input id="builder-name" maxlength="40" placeholder="e.g. Upper body" value="${escapeHtml(draft.name??(old?.name||''))}"></div>${type==='cardio'?`<div class="inline-fields"><div class="field"><label>Duration (min)</label><input id="builder-duration" type="number" min="1" value="${draft.duration??old?.duration??30}"></div><div class="field"><label>Distance (km)</label><input id="builder-distance" type="number" min="0" step="0.1" value="${draft.distance??old?.distance??''}"></div></div><div class="field"><label>Cardio mode</label><select id="builder-mode">${['Running','Cycling','Walking','Hiking','Rowing','Elliptical','Other'].map(m=>`<option ${m===(draft.mode??old?.mode??'Running')?'selected':''}>${m}</option>`).join('')}</select></div>`:`<section class="section"><div class="section-head"><h2>Exercises</h2><button class="text-btn" id="builder-add">+ ADD</button></div><div class="list" id="builder-items"></div></section>`}<div class="field"><label>Notes</label><textarea id="builder-notes" rows="4" maxlength="500" placeholder="Optional workout notes">${escapeHtml(draft.notes??old?.notes??'')}</textarea></div>${workoutVisibilityPicker(draft.visibility??old?.visibility??'private')}<div class="builder-actions"><button class="secondary" id="builder-cancel">CANCEL</button><button class="primary" id="builder-save">SAVE WORKOUT</button></div>`);
 if(type!=='cardio'){const paint=()=>{$('#builder-items').innerHTML=items.length?items.map((x,i)=>`<div class="exercise-row"><div class="exercise-row-head"><span class="drag">${i+1}</span><strong>${escapeHtml(x.name)}</strong><button class="small-btn builder-howto" data-builder-howto="${i}">HOW TO</button><button class="small-btn" data-up="${i}" ${i===0?'disabled':''}>↑</button><button class="small-btn" data-down="${i}" ${i===items.length-1?'disabled':''}>↓</button><button class="small-btn danger" data-remove="${i}">REMOVE</button></div><div class="inline-fields"><div class="field"><label>Sets</label><input type="number" min="1" max="20" value="${x.sets||3}" data-item-set="${i}"></div><div class="field"><label>${type==='rehab'?'Reps / time':'Reps'}</label><input value="${escapeHtml(x.reps||'8-12')}" data-item-reps="${i}"></div></div></div>`).join(''):`<div class="empty"><strong>No exercises added</strong>Add exercises to build this session.</div>`;$$('[data-builder-howto]').forEach(b=>b.onclick=()=>{const x=items[+b.dataset.builderHowto];if(x)openExerciseGuideByName(x.name,null,x.beginnerGuideKey||'',x)});$$('[data-remove]').forEach(b=>b.onclick=()=>{items.splice(+b.dataset.remove,1);paint()});$$('[data-up]').forEach(b=>b.onclick=()=>{const i=+b.dataset.up;if(i>0){[items[i-1],items[i]]=[items[i],items[i-1]];paint()}});$$('[data-down]').forEach(b=>b.onclick=()=>{const i=+b.dataset.down;if(i<items.length-1){[items[i+1],items[i]]=[items[i],items[i+1]];paint()}});$$('[data-item-set]').forEach(inp=>inp.oninput=()=>items[+inp.dataset.itemSet].sets=Math.max(1,+inp.value||1));$$('[data-item-reps]').forEach(inp=>inp.oninput=()=>items[+inp.dataset.itemReps].reps=inp.value)};paint();$('#builder-add').onclick=()=>{const currentDraft={name:$('#builder-name').value,notes:$('#builder-notes').value,visibility:selectedWorkoutVisibility()};openExercisePicker(type,x=>{items.push(x);openBuilder(type,editId,items,currentDraft)})}}
 $('#builder-cancel').onclick=closeSheet;$('#builder-save').onclick=async()=>{const name=$('#builder-name').value.trim();if(!name)return toast('Give the workout a name');if(type!=='cardio'&&!items.length)return toast('Add at least one exercise');const obj={...old,id:old?.id||uid(),name,type,items,notes:$('#builder-notes').value.trim(),visibility:selectedWorkoutVisibility(),createdAt:old?.createdAt||Date.now(),updatedAt:Date.now()};if(type==='cardio'){obj.duration=Math.max(1,+$('#builder-duration').value||30);obj.distance=Math.max(0,+$('#builder-distance').value||0);obj.mode=$('#builder-mode').value}if(old){data.customWorkouts=data.customWorkouts.map(w=>w.id===old.id?obj:w)}else data.customWorkouts.push(obj);syncPlannedWorkoutReferences();save();try{if(obj.visibility!=='private'||(old&&old.visibility!=='private'))await upsertSharedWorkoutCloud(obj);obj.cloudSyncPending=false;data.customWorkouts=data.customWorkouts.map(w=>w.id===obj.id?obj:w);save()}catch(e){console.error('workout share sync',e);obj.cloudSyncPending=true;data.customWorkouts=data.customWorkouts.map(w=>w.id===obj.id?obj:w);save();if(obj.visibility!=='private')toast('Workout saved locally. Sharing could not sync yet.')}resetWorkoutShareState();closeSheet();render();toast(old?'Workout updated everywhere':'Workout saved')};}
...
function persistActiveSession(){if(data.activeSession){data.activeSession.modifiedAt=Date.now();writeActiveSessionRecovery(activeDataUserId,data.activeSession);save()}}
...
function coachMarkup(){const c=getCoachInsight(),check=latestCoachCheckin(),label=c.status==='ready'?'READY':c.status==='recovery'?'RECOVERY':'CAUTION';return `<button class="card coach-home-card ${c.status}" id="coach-why"><span class="coach-home-icon">C</span><span class="coach-home-copy"><span class="coach-home-topline"><small>GAYM COACH · ${escapeHtml(c.topic)}</small><em class="coach-status-badge ${c.status}">${label}</em></span><strong>${escapeHtml(c.headline)}</strong><span>${escapeHtml(c.reason)}</span><i>${check?'Check-in updated today':'No recovery check-in today'}</i></span><b>›</b></button>`}
...
function bottomHomeMarkup(){const check=latestBottomCheckin(),result=check?bottomAssessment(check):null;const label=result?(result.level==='good'?'GOOD TO GO':result.level==='maybe'?'CAUTION':'WAIT'):'NOT CHECKED';const status=`<span class="bottom-home-status ${result?.level||'unknown'}">${label}</span>`;return `<button class="card bottom-home-card ${result?.level||'unknown'}" id="bottom-check" type="button"><span class="bottom-home-icon">🍑</span><span class="grow"><span class="bottom-home-kicker">BOTTOM CHECK · TODAY</span><strong>${result?escapeHtml(result.title):'Quick gut check'}</strong><small>${result?escapeHtml(result.subtitle):'Not checked today. Tap to update your status.'}</small></span><span class="bottom-home-right">${status}<span class="chev">›</span></span></button>`}
...
function openCoachWhy(){
 ...
 let feeling=check?.feeling||'normal',soreness=check?.soreness||'none';function commit(){data.coachCheckins=(data.coachCheckins||[]).filter(x=>x.date!==isoToday());data.coachCheckins.push({date:isoToday(),feeling,soreness,at:Date.now()});save();refreshUnderlyingView();openCoachWhy()}$$('[data-feeling]').forEach(b=>b.onclick=()=>{feeling=b.dataset.feeling;commit()});$$('[data-soreness]').forEach(b=>b.onclick=()=>{soreness=b.dataset.soreness;commit()})
}
...
function startWorkoutTemplate(w){activeExerciseOpen=0;data.activeSession={id:uid(),workoutId:w.id||'',name:w.name,type:w.type||'strength',program:w.program||'',programDay:w.programDay||null,beginner:!!w.beginner,startedAt:Date.now(),modifiedAt:Date.now(),date:isoToday(),notes:w.notes||'',duration:w.duration||0,distance:w.distance||0,mode:w.mode||'',items:(w.items||[]).map(x=>({name:x.name,muscle:x.muscle||'',equipment:x.equipment||'',targetReps:x.reps||'',beginnerGuideKey:x.beginnerGuideKey||'',note:x.note||'',sets:Array.from({length:x.sets||3},()=>({weight:'',reps:'',done:false}))}))};save();go('active')}
...
function active(){
 const s=data.activeSession;if(!s){go('workout');return}if(s.type==='cardio'){const legacy=structuredClone(s);markActiveSessionClosed(s.id);data.activeSession=null;save();go('workout');setTimeout(()=>openLogCardio({activity:legacy.mode||legacy.name||'Cardio',duration:legacy.duration||'',distance:legacy.distance||'',notes:legacy.notes||''}),0);return}const mins=sessionElapsedMinutes(s);
 ...
 screen.oninput=e=>{const target=e.target;if(target.matches('[data-weight]')){const[i,j]=target.dataset.weight.split(':').map(Number);if(s.items[i]?.sets?.[j]){s.items[i].sets[j].weight=target.value;save()}}else if(target.matches('[data-reps]')){const[i,j]=target.dataset.reps.split(':').map(Number);if(timedSetTimer&&timedSetTimer.sessionId===s.id&&timedSetTimer.itemIndex===i&&timedSetTimer.setIndex===j)stopTimedSetTimer({saveElapsed:false});const set=s.items[i]?.sets?.[j];if(set){set.reps=target.value;if(Number(target.value)>0&&set.manuallySkipped!==true)set.done=true;save();const row=target.closest('.set-row'),check=row?.querySelector('[data-check]');row?.classList.toggle('set-done',!!set.done);if(check){check.classList.toggle('done',!!set.done);check.textContent=set.done?'✓':'○';check.setAttribute('aria-pressed',set.done?'true':'false')}updateActiveProgressUI(s)}}else if(target.matches('[data-note]')){const i=+target.dataset.note;if(s.items[i]){s.items[i].note=target.value;save()}}};
...
let matvareFoodsCache=null,matvareFoodsPromise=null,nutritionSearchTimer=null,activeBarcodeScanner=null,mealBuilderDraft=null;
...
const MEAL_TYPE_ALIASES={
 Breakfast:['breakfast','breakfasts','frukost','frukosten','frokost','frokosten','morgenmad','morgonmal','morgonmål'],
 Lunch:['lunch','lunchen','lunsj','lunsjen','frokost lunch'],
 Dinner:['dinner','dinners','middag','middagen','middagmat','kvallsmat','kvällsmat','kveldsmat','supper','evening meal'],
 Snack:['snack','snacks','mellanmål','mellanmal','mellommåltid','mellommaltid','mellommåltider','mellommal','mellommål','kveldsmat snack']
};
function canonicalMealType(value,fallback='Snack'){
 const raw=foodSearchNorm(value||'');if(!raw)return fallback;
 for(const type of mealTypes())if(foodSearchNorm(type)===raw)return type;
 for(const [type,aliases] of Object.entries(MEAL_TYPE_ALIASES))if(aliases.some(a=>foodSearchNorm(a)===raw))return type;
 // Be forgiving with labels such as "Breakfast / Frukost" or "Frokost 08:00".
 for(const [type,aliases] of Object.entries(MEAL_TYPE_ALIASES))if(aliases.some(a=>{const n=foodSearchNorm(a);return raw===n||raw.startsWith(n+' ')||raw.endsWith(' '+n)}))return type;
 return fallback;
}
function mealTypeFromEntry(n,fallback='Snack'){return canonicalMealType(n?.mealType??n?.meal??n?.mealName??n?.category??n?.type,fallback)}
function normalizeNutritionMealTypes(){let changed=false;(data.nutrition||[]).forEach(n=>{const next=mealTypeFromEntry(n,'Snack');if(n.mealType!==next){n.mealType=next;changed=true}});(data.savedMeals||[]).forEach(m=>{const next=canonicalMealType(m?.mealType??m?.name,'Snack');if(m.mealType!==next){m.mealType=next;changed=true}});return changed}
function mealTypeOptions(selected=guessMealType()){selected=canonicalMealType(selected,guessMealType());return mealTypes().map(x=>`<option value="${x}" ${x===selected?'selected':''}>${x}</option>`).join('')}
function logFoodTemplate(food,amount,mealType=guessMealType(),date=isoToday()){const scaled=nutritionScale(food.per100,amount),entry={id:uid(),date,mealType:canonicalMealType(mealType,guessMealType()),name:food.name,brand:food.brand||'',source:food.source||'manual',sourceId:food.sourceId||'',foodId:food.id||'',amount:n1(amount),unit:food.unit||'g',per100:structuredClone(food.per100),...scaled,fiberProvided:true};data.nutrition.push(entry);save();return entry}
function openFoodPortionSheet(food,{amount=100,mealType=guessMealType(),afterLog=null,onAdd=null,returnToBuilder=false}={}){
 if(!food?.per100)return toast('Nutrition values are missing for this food.');const src=FOOD_SOURCE_LABELS[food.source]||food.source||'Food source',fav=favoriteFoodTemplate(food),building=typeof onAdd==='function';
 openSheet(`<div class="sheet-head"><div><p class="eyebrow">${escapeHtml(src)}</p><h2>${escapeHtml(food.name)}</h2>${food.brand?`<p class="subtle">${escapeHtml(food.brand)}</p>`:''}</div><button class="sheet-close" data-close>×</button></div><article class="nutrition-source-card"><div><span>PER 100 ${escapeHtml(food.unit||'g')}</span><strong>${n0(food.per100.kcal)} kcal</strong></div><div><span>PROTEIN</span><strong>${n1(food.per100.protein)} g</strong></div><div><span>CARBS</span><strong>${n1(food.per100.carbs)} g</strong></div><div><span>FAT</span><strong>${n1(food.per100.fat)} g</strong></div></article><div class="${building?'field':'inline-fields'}"><div class="field"><label>Amount (${escapeHtml(food.unit||'g')})</label><input id="portion-amount" type="number" inputmode="decimal" min="0.1" step="1" value="${Number(amount)||100}"></div>${building?'':`<div class="field"><label>Meal</label><select id="portion-meal">${mealTypeOptions(mealType)}</select></div>`}</div><article class="nutrition-live-total" id="portion-total"></article><div class="food-source-note">${food.source==='openfoodfacts'?`Barcode data can be community-contributed. Values are normalized per 100 ${escapeHtml(food.unit||'g')}; compare them with the package label if anything looks wrong.`:'Values are calculated per 100 g. Choose the amount you actually used.'}</div><div class="sheet-actions"><button class="secondary" id="portion-fav">${fav?'★ FAVORITE':'☆ ADD FAVORITE'}</button><button class="primary" id="portion-log">${building?'ADD TO MEAL':'LOG FOOD'}</button></div>`);
 const amountEl=$('#portion-amount'),total=$('#portion-total');const paint=()=>{const x=nutritionScale(food.per100,+amountEl.value||0);total.innerHTML=`<strong>${x.kcal} kcal</strong><span>${x.protein} g protein · ${x.carbs} g carbs · ${x.fat} g fat · ${x.fiber} g fiber</span>`};paint();amountEl.oninput=paint;$('#portion-fav').onclick=()=>{$('#portion-fav').textContent=toggleFoodFavorite(food)?'★ FAVORITE':'☆ ADD FAVORITE'};$('#portion-log').onclick=()=>{const a=+amountEl.value;if(!a||a<=0)return toast('Enter the amount you used.');if(building){onAdd(food,a);closeSheet();if(returnToBuilder)openMealBuilder();return}logFoodTemplate(food,a,$('#portion-meal').value);closeSheet();nutrition();toast(`${food.name} logged`);afterLog?.()};
}
function openNutritionSearch(initial='',{onSelect=null}={}){
 openSheet(`<div class="sheet-head"><div><p class="eyebrow">Matvaretabellen · Mattilsynet</p><h2>${onSelect?'Add ingredient':'Search food'}</h2></div><button class="sheet-close" data-close>×</button></div><div class="food-search-box"><input id="food-db-search" value="${escapeHtml(initial)}" autocomplete="off" placeholder="e.g. karbonadedeig, havregryn, ris…"><span id="food-db-status">Type at least 2 letters</span></div><div id="food-db-results" class="food-search-results"></div><p class="science-note">Generic foods use the Norwegian Food Composition Table. You choose the exact database match and amount before anything is saved.</p>`);
 const input=$('#food-db-search'),results=$('#food-db-results'),status=$('#food-db-status');let epoch=0;const run=async()=>{const q=input.value.trim(),my=++epoch;if(q.length<2){results.innerHTML='';status.textContent='Type at least 2 letters';return}status.textContent='Searching Matvaretabellen…';try{const items=await searchMatvareFoods(q);if(my!==epoch)return;status.textContent=items.length?`${items.length} matches`:'No match';results.innerHTML=items.length?items.map((f,i)=>`<button class="food-result-card" data-food-result="${i}"><span><strong>${escapeHtml(f.name)}</strong><small>${n0(f.per100.kcal)} kcal · ${n1(f.per100.protein)} g protein per 100 g</small></span><b>›</b></button>`).join(''):`<div class="empty"><strong>No food found</strong>Try a shorter Norwegian food name or use Manual label values.</div>`;$$('[data-food-result]').forEach(b=>b.onclick=()=>{const f=items[+b.dataset.foodResult];closeSheet();openFoodPortionSheet(f,onSelect?{onAdd:onSelect,returnToBuilder:true}:{})})}catch(e){if(my!==epoch)return;status.textContent='Could not load Matvaretabellen';results.innerHTML=`<div class="empty"><strong>Food database unavailable</strong>${escapeHtml(e.message||'Check your internet connection.')}</div>`}};input.oninput=()=>{clearTimeout(nutritionSearchTimer);nutritionSearchTimer=setTimeout(run,250)};if(initial){input.focus();run()}else input.focus();
}
...
async function openBarcodeScanner({onSelect=null}={}){
 ...
 if(globalThis.Html5Qrcode){
  try{
   ...
   activeBarcodeScanner=new Html5Qrcode('barcode-reader',{formatsToSupport:formats,verbose:false});let handled=false;
   const config={fps:10,qrbox:{width:250,height:120},aspectRatio:1.777778,disableFlip:true};
   activeBarcodeScanner.start({facingMode:{exact:'environment'}},config,code=>{if(handled)return;handled=true;lookup(code)},()=>{}).then(()=>{if(status)status.textContent='Point the rear camera at the barcode'}).catch(async()=>{
    // Some Safari/WebKit versions reject exact constraints even when a rear camera exists.
    try{await activeBarcodeScanner.start({facingMode:'environment'},config,code=>{if(handled)return;handled=true;lookup(code)},()=>{});if(status)status.textContent='Point the rear camera at the barcode'}catch(e){console.error('rear barcode camera',e);if(status)status.textContent='Rear camera unavailable · enter barcode below'}
   });
  }catch(e){console.error('barcode scanner',e);if(status)status.textContent='Rear camera unavailable · enter barcode below'}
 }else if(status)status.textContent='Camera scanner could not load · enter barcode below';
}
...
function openManualFoodSheet({onSelect=null}={}){openSheet(`<div class="sheet-head"><div><p class="eyebrow">Manual label</p><h2>Add product values</h2></div><button class="sheet-close" data-close>×</button></div><p class="subtle">Enter exactly what the package says per 100 g. Then choose how many grams you used.</p><div class="field"><label>Product name</label><input id="mf-name" placeholder="e.g. Mutti crushed tomatoes"></div><div class="inline-fields"><div class="field"><label>kcal / 100 g</label><input id="mf-kcal" type="number" step="0.1"></div><div class="field"><label>Protein / 100 g</label><input id="mf-protein" type="number" step="0.1"></div></div><div class="inline-fields"><div class="field"><label>Carbs / 100 g</label><input id="mf-carbs" type="number" step="0.1"></div><div class="field"><label>Fat / 100 g</label><input id="mf-fat" type="number" step="0.1"></div></div><div class="field"><label>Fiber / 100 g</label><input id="mf-fiber" type="number" step="0.1" value="0"></div><button class="primary" id="mf-next">CHOOSE AMOUNT</button>`);$('#mf-next').onclick=()=>{const name=$('#mf-name').value.trim(),kcal=+$('#mf-kcal').value;if(!name||!Number.isFinite(kcal)||kcal<0)return toast('Add a name and kcal per 100 g.');const food={id:`manual:${uid()}`,source:'manual',sourceId:'',name,brand:'',unit:'g',per100:{kcal:n1(kcal),protein:n1($('#mf-protein').value),carbs:n1($('#mf-carbs').value),fat:n1($('#mf-fat').value),fiber:n1($('#mf-fiber').value)}};closeSheet();openFoodPortionSheet(food,onSelect?{onAdd:onSelect,returnToBuilder:true}:{})}}

function mealBuilderItem(food,amount){return {id:uid(),food:structuredClone(food),amount:n1(amount)}}
function mealBuilderTotals(){const items=mealBuilderDraft?.items||[];return items.reduce((t,x)=>{const m=nutritionScale(x.food.per100,x.amount);t.kcal+=m.kcal;t.protein+=m.protein;t.carbs+=m.carbs;t.fat+=m.fat;t.fiber+=m.fiber;return t},{kcal:0,protein:0,carbs:0,fat:0,fiber:0})}
function addToMealBuilder(food,amount){if(!mealBuilderDraft)mealBuilderDraft={name:'',mealType:guessMealType(),items:[]};mealBuilderDraft.items.push(mealBuilderItem(food,amount))}
function startMealBuilder({scanFirst=false}={}){if(!mealBuilderDraft)mealBuilderDraft={name:'',mealType:guessMealType(),items:[]};if(scanFirst)return openBarcodeScanner({onSelect:addToMealBuilder});openMealBuilder()}
function openMealBuilder(){if(!mealBuilderDraft)mealBuilderDraft={name:'',mealType:guessMealType(),items:[]};const d=mealBuilderDraft,t=mealBuilderTotals();openSheet(`<div class="sheet-head"><div><p class="eyebrow">Meal builder</p><h2>Build the whole meal</h2></div><button class="sheet-close" data-close>×</button></div><p class="subtle">Add every ingredient first. Nothing is written to today until you press <strong>LOG MEAL</strong>.</p><div class="inline-fields"><div class="field"><label>Meal name</label><input id="mb-name" value="${escapeHtml(d.name||'')}" placeholder="e.g. Pasta & meat sauce"></div><div class="field"><label>Meal</label><select id="mb-type">${mealTypeOptions(d.mealType)}</select></div></div><div class="meal-builder-actions"><button class="secondary" id="mb-scan">SCAN BARCODE</button><button class="secondary" id="mb-search">SEARCH FOOD</button><button class="secondary" id="mb-manual">LABEL VALUES</button></div><div class="meal-builder-items">${d.items.length?d.items.map((x,i)=>{const m=nutritionScale(x.food.per100,x.amount);return `<article class="meal-builder-item"><div><strong>${escapeHtml(x.food.name)}</strong><small>${escapeHtml(x.food.brand||FOOD_SOURCE_LABELS[x.food.source]||x.food.source||'Food')}</small></div><label><input data-mb-amount="${i}" type="number" min="0.1" step="1" value="${x.amount}"><span>${escapeHtml(x.food.unit||'g')}</span></label><b>${m.kcal} kcal</b><button class="text-btn danger" data-mb-remove="${i}">REMOVE</button></article>`}).join(''):`<div class="empty meal-builder-empty"><strong>No ingredients yet</strong>Scan a package, search Matvaretabellen, or enter label values.</div>`}</div><article class="meal-builder-total"><div><span>TOTAL MEAL</span><strong>${Math.round(t.kcal)} kcal</strong></div><p>${n1(t.protein)} g protein · ${n1(t.carbs)} g carbs · ${n1(t.fat)} g fat · ${n1(t.fiber)} g fiber</p></article><div class="sheet-actions"><button class="primary" id="mb-log" ${d.items.length?'':'disabled'}>LOG MEAL</button><button class="secondary" id="mb-save" ${d.items.length?'':'disabled'}>SAVE FOR LATER</button><button class="text-btn danger" id="mb-clear" ${d.items.length?'':'disabled'}>CLEAR DRAFT</button></div>`);
 const capture=()=>{d.name=$('#mb-name')?.value.trim()||'';d.mealType=canonicalMealType($('#mb-type')?.value,guessMealType())};
 $$('[data-mb-amount]').forEach(el=>el.onchange=()=>{capture();const i=+el.dataset.mbAmount,a=+el.value;if(a>0&&d.items[i])d.items[i].amount=n1(a);openMealBuilder()});
 $$('[data-mb-remove]').forEach(b=>b.onclick=()=>{capture();d.items.splice(+b.dataset.mbRemove,1);openMealBuilder()});
 $('#mb-scan').onclick=()=>{capture();openBarcodeScanner({onSelect:addToMealBuilder})};$('#mb-search').onclick=()=>{capture();openNutritionSearch('',{onSelect:addToMealBuilder})};$('#mb-manual').onclick=()=>{capture();openManualFoodSheet({onSelect:addToMealBuilder})};
 $('#mb-log').onclick=()=>{capture();if(!d.items.length)return;const mealId=uid(),mealName=d.name||d.mealType,date=isoToday();const entries=d.items.map(x=>{const scaled=nutritionScale(x.food.per100,x.amount);return {id:uid(),date,mealType:d.mealType,mealId,mealName,name:x.food.name,brand:x.food.brand||'',source:x.food.source||'manual',sourceId:x.food.sourceId||'',foodId:x.food.id||'',amount:n1(x.amount),unit:x.food.unit||'g',per100:structuredClone(x.food.per100),...scaled,fiberProvided:true}});data.nutrition.push(...entries);save();mealBuilderDraft=null;closeSheet();nutrition();toast(`${mealName} logged · ${entries.length} ingredients`)};
 $('#mb-save').onclick=()=>{capture();if(!d.items.length)return;const meal={id:uid(),name:d.name||`${d.mealType} meal`,mealType:d.mealType,items:d.items.map(x=>({...structuredClone(x.food),amount:x.amount}))};data.savedMeals=[...(data.savedMeals||[]),meal];save();toast(`${meal.name} saved for later`);openMealBuilder()};const clear=$('#mb-clear');if(clear)clear.onclick=()=>{mealBuilderDraft=null;closeSheet();nutrition();toast('Meal draft cleared')};
}
...
function openNutritionEntryEdit(id){const n=(data.nutrition||[]).find(x=>String(x.id)===String(id));if(!n?.per100)return;const food={id:n.foodId||n.sourceId||n.id,source:n.source,sourceId:n.sourceId,name:n.name,brand:n.brand||'',unit:n.unit||'g',per100:structuredClone(n.per100)};openSheet(`<div class="sheet-head"><div><p class="eyebrow">Edit amount</p><h2>${escapeHtml(n.name)}</h2></div><button class="sheet-close" data-close>×</button></div><div class="inline-fields"><div class="field"><label>Amount (${escapeHtml(n.unit||'g')})</label><input id="edit-food-amount" type="number" step="1" value="${n.amount||100}"></div><div class="field"><label>Meal</label><select id="edit-food-meal">${mealTypeOptions(mealTypeFromEntry(n,guessMealType()))}</select></div></div><article class="nutrition-live-total" id="edit-food-total"></article><button class="primary" id="edit-food-save">SAVE CHANGES</button>`);const a=$('#edit-food-amount'),t=$('#edit-food-total'),paint=()=>{const x=nutritionScale(food.per100,+a.value||0);t.innerHTML=`<strong>${x.kcal} kcal</strong><span>${x.protein} g protein · ${x.carbs} g carbs · ${x.fat} g fat</span>`};paint();a.oninput=paint;$('#edit-food-save').onclick=()=>{const amount=+a.value;if(!amount||amount<=0)return toast('Enter an amount.');Object.assign(n,{amount:n1(amount),mealType:canonicalMealType($('#edit-food-meal').value,guessMealType()),...nutritionScale(food.per100,amount)});save();closeSheet();nutrition();toast('Food updated')}}

function loggedMealRows(items){const groups=[],byId=new Map();for(const n of items){if(n.mealId){let g=byId.get(String(n.mealId));if(!g){g={id:String(n.mealId),name:n.mealName||mealTypeFromEntry(n,'Meal'),items:[]};byId.set(String(n.mealId),g);groups.push(g)}g.items.push(n)}else groups.push({id:null,items:[n]})}return groups.map(g=>g.id?loggedMealCard(g):nutritionMealCard(g.items[0])).join('')}
function loggedMealCard(g){const t=g.items.reduce((a,n)=>{a.kcal+=+n.kcal||0;a.protein+=+n.protein||0;return a},{kcal:0,protein:0});return `<article class="list-card logged-meal-card"><div><h3>${escapeHtml(g.name||'Meal')}</h3><p>${g.items.length} ingredients · ${n1(t.protein)} g protein</p></div><b>${Math.round(t.kcal)} kcal</b><div class="logged-meal-actions"><button class="small-btn" data-edit-meal="${escapeHtml(g.id)}">EDIT MEAL</button></div></article>`}
function openLoggedMeal(mealId){
 const items=(data.nutrition||[]).filter(n=>String(n.mealId||'')===String(mealId));if(!items.length)return;
 const currentName=items[0].mealName||mealTypeFromEntry(items[0],'Meal'),currentType=mealTypeFromEntry(items[0],guessMealType());
 openSheet(`<div class="sheet-head"><div><p class="eyebrow">Logged meal</p><h2>${escapeHtml(currentName)}</h2></div><button class="sheet-close" data-close>×</button></div><div class="inline-fields"><div class="field"><label>Meal name</label><input id="logged-meal-name" value="${escapeHtml(currentName)}"></div><div class="field"><label>Meal</label><select id="logged-meal-type">${mealTypeOptions(currentType)}</select></div></div><div class="meal-builder-items">${items.map(n=>`<article class="meal-builder-item"><div><strong>${escapeHtml(n.name)}</strong><small>${escapeHtml(FOOD_SOURCE_LABELS[n.source]||n.source||'Food')}</small></div><label><input data-logmeal-amount="${n.id}" type="number" min="0.1" step="1" value="${n.amount||100}"><span>${escapeHtml(n.unit||'g')}</span></label><b>${Math.round(+n.kcal||0)} kcal</b><button class="text-btn danger" data-logmeal-remove="${n.id}">REMOVE</button></article>`).join('')}</div><div class="sheet-actions"><button class="primary" id="logged-meal-done">SAVE MEAL</button></div>`);
 const captureMeta=()=>{const name=$('#logged-meal-name')?.value.trim()||currentName,type=canonicalMealType($('#logged-meal-type')?.value,currentType);items.forEach(n=>{n.mealName=name;n.mealType=type});return {name,type}};
 $$('[data-logmeal-amount]').forEach(el=>el.onchange=()=>{captureMeta();const n=items.find(x=>String(x.id)===String(el.dataset.logmealAmount)),a=+el.value;if(!n?.per100||!a||a<=0)return;Object.assign(n,{amount:n1(a),...nutritionScale(n.per100,a)});save();openLoggedMeal(mealId)});
 $$('[data-logmeal-remove]').forEach(b=>b.onclick=()=>{captureMeta();removeNutritionWhere(n=>String(n.id)===String(b.dataset.logmealRemove));save();const left=(data.nutrition||[]).some(n=>String(n.mealId||'')===String(mealId));if(left)openLoggedMeal(mealId);else{closeSheet();nutrition()}});
 $('#logged-meal-done').onclick=()=>{captureMeta();save();closeSheet();nutrition();toast('Meal updated')}
}
// ---------------------------------------------------------------------------
function nutrition(){if(normalizeNutritionMealTypes())save();
 ...
 const groups=mealTypes().map(type=>[type,f.filter(n=>mealTypeFromEntry(n,'Snack')===type)]).filter(x=>x[1].length);
 ...
 <section class="section nutrition-fastlog"><p class="eyebrow">FAST LOG</p><button class="quick-log-hero" id="quick-log"><span>What did you eat?</span><strong>“200 g beef + 125 g rice…”</strong><b>›</b></button><div class="nutrition-action-grid"><button class="primary" id="build-meal">${mealBuilderDraft?.items?.length?`RESUME MEAL (${mealBuilderDraft.items.length})`:'BUILD MEAL'}</button><button class="secondary" id="scan-food">${mealBuilderDraft?.items?.length?'SCAN MORE':'SCAN INTO MEAL'}</button><button class="secondary" id="search-food">SEARCH FOOD</button><button class="secondary" id="copy-yesterday" ${yesterday.length?'':'disabled'}>COPY YESTERDAY</button></div></section>
 ...
 <section class="section"><div class="section-head"><h2>Today's meals</h2></div>${groups.length?groups.map(([type,items])=>`<div class="nutrition-meal-group"><div class="nutrition-meal-head"><h3>${type}</h3><button class="text-btn" data-save-meal="${type}">SAVE MEAL</button></div><div class="list">${loggedMealRows(items)}</div></div>`).join(''):`<div class="empty"><strong>No food logged today</strong>Search Matvaretabellen, scan a package or use Quick Add.</div>`}</section>
 ...
 if(rows.length){
  ...
  if(up.error)console.error('activity sync upsert',up.error);
   ...
   for(const post of up.data||[]){
    ...
    const oldPrs=previousWorkouts.get(String(post.source_id))?.metadata?.prs||[],newPrs=post.metadata?.prs||[];
    const oldKeys=new Set(oldPrs.map(pr=>`${String(pr.name||'').toLowerCase()}|${Number(pr.weight)||0}|${Number(pr.reps)||0}`));
    const hasNewPb=newPrs.some(pr=>!oldKeys.has(`${String(pr.name||'').toLowerCase()}|${Number(pr.weight)||0}|${Number(pr.reps)||0}`));
    if(!hasNewPb)continue;
    const rels=await sb.from('friendships').select('requester_id,addressee_id,status').eq('status','accepted').or(`requester_id.eq.${expectedUserId},addressee_id.eq.${expectedUserId}`);
    if(rels.error){console.error('PB friend lookup',rels.error);continue}
    const actorName=cloudProfile?.display_name||cloudProfile?.username||data.profile.name||'Your friend';
    const friendIds=[...new Set((rels.data||[]).map(r=>r.requester_id===expectedUserId?r.addressee_id:r.requester_id).filter(Boolean))];
    await Promise.all(friendIds.map(friendId=>sendSocialNotification(friendId,'pr',`${actorName} hit a PB`,'Check it out in Activity.',{post_id:post.id,target_type:'workout'},`pb-workout:${post.id}:${friendId}`)));
   }
  }
 }
 if(authUser?.id!==expectedUserId||activeDataUserId!==expectedUserId)return;
...
function closeSheet(){
 if(activeBarcodeScanner)stopBarcodeScanner();
