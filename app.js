...
async function navigateNotification(n){
 if(n.social){await markSocialNotificationRead(n.id);closeSheet();if(n.kind==='friend_request'){socialTab='friends';go('profile');return}if(n.metadata?.post_id){await openActivityPostById(n.metadata.post_id);return}if(n.actor_id&&n.actor_id!==authUser?.id){openUserProfile(n.actor_id);return}go('home');return}
...
function socialWorkoutMetadata(sess,detailLevel='full',sharePrs=true){
 ...
 const meta={date:sess.date||null,type:sess.type||'strength',durationMin:Math.max(0,Math.round(Number(sess.durationMin||0))),exerciseCount:(sess.items||[]).length,detail_level:detailLevel};
...
async function loadSocialState(force=false){
 ...
 socialLoadPromise=(async()=>{
  const [{data:rels,error:relsError},{data:posts,error:postsError},{data:shares,error:sharesError},{data:notifs,error:notifError}]=await Promise.all([
   ...
   sb.from('activity_posts').select('*,profiles:user_id(id,username,display_name,avatar_url),activity_kudos(user_id,profiles:user_id(id,username,display_name,avatar_url)),activity_comments(id,user_id,body,created_at,profiles:user_id(username,display_name,avatar_url))').order('created_at',{ascending:false}).limit(30),
...
function renderSocialTab(){
 ...
 if(socialTab==='activity'){
  box.innerHTML=`<div class="section-head"><h2>Friends activity</h2><span class="eyebrow">KUDOS + COMMENTS</span></div><div class="social-feed">${socialCache.feed.length?socialCache.feed.map(activityCard).join(''):'<div class="empty"><strong>Your feed is quiet</strong>Add friends and their shared workouts will appear here.</div>'}</div>`;bindActivityCards();
